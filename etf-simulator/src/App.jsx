    ...
    <nav style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "0 20px", position: "relative" }}>
      <div style={{ display: "flex", alignItems: "center", height: 52, justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {user ? (
            <><span className="hide-mobile" style={{ color: C.sub, fontSize: 12.5 }}><span style={{ color: C.green }}>{"●"}</span> {user.username || user.name}</span><button onClick={signOut} style={{ ...btnO(), fontSize: 12, padding: "5px 12px" }}>Sign Out</button></>
          ) : (
                          ...
                          <>
                            ...
                            {[...txs].reverse().map((tx, i) => (
                              <div key={i} style={{ display: "grid", gridTemplateColumns: "70px 70px 90px 80px 80px 1fr", padding: "10px 20px", borderBottom: `1px solid ${C.border}`, alignItems: "start", fontSize: 12, minWidth: 520, background: i === 0 && tx.status === "FILLED" ? `${C.green}06` : "transparent" }}>
                                <span style={{ ...badge(tx.type === "BUY" ? C.green : tx.type === "SELL" || tx.type === "TRIM" || tx.type === "LIQUIDATE" || tx.type === "AUTO-SELL" ? C.red : tx.type === "SHORT" ? "#f59e0b" : tx.type === "FEE" ? C.dim : C.teal), fontSize: 9 }}>{tx.type}</span>
                                <div>
              ...
              {isOpen && i < 10 && (
                <div style={{ borderTop: `1px solid ${C.border}`, background: C.surface }}>
                  ...
                  {lbTab === "holdings" && p.holdings && (
                    <div className="mob-scroll" style={{ padding: 0 }}>
                      <div style={{ display: "grid", gridTemplateColumns: "38px 50px 1fr 70px 90px 1fr", padding: "8px 18px", borderBottom: `1px solid ${C.border}`, fontSize: 9, color: C.dim, fontFamily: mono, textTransform: "uppercase", letterSpacing: 0.4, minWidth: 480 }}>
                        <span>#</span><span>Type</span><span>Holding</span><span>Weight</span><span>Value</span><span>Role</span>
                      </div>
                      {p.holdings.map((h, hi) => (
                        <div key={hi} style={{ display: "grid", gridTemplateColumns: "38px 50px 1fr 70px 90px 1fr", padding: "10px 18px", borderBottom: `1px solid ${C.border}`, alignItems: "center", fontSize: 12, minWidth: 480 }}>
                          ...
                          <span style={{ color: C.accent, fontFamily: mono, fontWeight: 700, fontSize: 12 }}>{fmt(h.weight, 1)}%</span>
                          <span style={{ color: C.text, fontFamily: mono, fontSize: 11 }}>{fmtUSD(h.allocation)}</span>
                          <div>
                            <span style={{ color: C.sub, fontSize: 10 }}>{h.role || "—"}</span>
                            {h.conviction && <span style={{ color: h.conviction === "high" ? C.green : h.conviction === "low" ? C.red : C.gold, fontSize: 9, marginLeft: 6, fontFamily: mono }}>({h.conviction})</span>}
                          </div>
                        </div>
                      ))}
                    </div>
                        ...
                        <>
                          ...
                          {[...p.transactions].reverse().map((tx, ti) => (
                            <div key={ti} style={{ display: "grid", gridTemplateColumns: "60px 70px 80px 70px 1fr", padding: "8px 18px", borderBottom: `1px solid ${C.border}`, alignItems: "start", fontSize: 11, minWidth: 420 }}>
                              <span style={{ ...badge(tx.type === "BUY" ? C.green : tx.type === "SELL" || tx.type === "AUTO-SELL" ? C.red : tx.type === "FEE" ? C.dim : C.teal), fontSize: 8 }}>{tx.type}</span>
...
export default function App() {
  ...
  const livePublicPortfolios = useMemo(() => {
    ...
    return portfolios.filter(p => p.isPublic).map(p => ({
      id: p.id, name: p.name, creator: user.username || user.name, value: p.value,

...
function ShareMenu({ text, compact, label, style: customStyle }) {
  ...
  const btnBase = { background: "transparent", border: "none", color: C.sub, cursor: "pointer", fontSize: 11, fontFamily: "inherit", padding: "6px 10px", borderRadius: 4, display: "flex", alignItems: "center", gap: 6, width: "100%", textAlign: "left" };
  if (compact) {
    return (
      <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
        <button onClick={(e) => { e.stopPropagation(); setOpen(!open); }} style={{ background: C.surface, border: `1px solid ${C.border}`, color: C.sub, borderRadius: 6, padding: "4px 10px", fontSize: 10, cursor: "pointer", fontFamily: mono, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 4, ...customStyle }}>↗ {label || "Share"}</button>
        {open && <div style={{ position: "absolute", top: "100%", right: 0, marginTop: 4, background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: 4, zIndex: 50, minWidth: 160, boxShadow: "0 8px 24px rgba(0,0,0,.4)" }}>
          <button onClick={() => { shareToX(text); setOpen(false); }} style={{ ...btnBase, color: C.text }}><span style={{ fontSize: 13, fontWeight: 800 }}>𝕏</span> Post to X</button>
          <button onClick={() => { shareToFacebook(text); setOpen(false); }} style={{ ...btnBase, color: "#1877F2" }}><span style={{ fontSize: 13 }}>f</span> Facebook</button>
          <button onClick={() => { handleCopy(); }} style={{ ...btnBase, color: copied ? C.green : C.sub }}>{copied ? "✓ Copied!" : "📋 Copy for Instagram / LinkedIn"}</button>
        </div>}
      </div>
    );
  }
  return (
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", ...customStyle }}>
      <button onClick={() => shareToX(text)} style={{ background: "#000", color: "#fff", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 }}><span style={{ fontSize: 14 }}>𝕏</span> Share to X</button>
      <button onClick={() => shareToFacebook(text)} style={{ background: "#1877F2", color: "#fff", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 }}>f Facebook</button>
      <button onClick={handleCopy} style={{ background: C.surface, color: copied ? C.green : C.sub, border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 16px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 }}>{copied ? "✓ Copied!" : "📋 Copy"}</button>
    </div>
  );
}

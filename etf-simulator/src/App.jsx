...
const fmtPrice = (v) => {
  ...
  if (abs >= 100) return "$" + v.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 });
...
- Use the FULL universe: niche ETFs (KWEB, ARKG, BETZ, BLOK, DFEN, XBI, IBB, PAVE, IFRA), small/mid-cap stocks, international ADRs (BABA, TSM, ASML, SAP, SHOP, SE, MELI, NU, GRAB), sector-specific crypto (AAVE, UNI, SOL, LINK, FET, NEAR, INJ, TAO), and specialty commodity plays (URA, COPX, REMX, LIT, WOOD).
  ...
  return (
    <div className="page-container" style={{ maxWidth: 1060, margin: "0 auto", padding: "48px 20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14, marginBottom: 52, textAlign: "center" }}>
        {[
          { n: "10,000+", l: "US Stocks & ETFs" }, { n: "141", l: "Crypto Assets" }, { n: "$1M", l: "Simulated Capital" }, { n: "Live", l: "Finnhub Market Data" },
        ].map((s) => (
      ...
      <div className="grid3" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 52 }}>
        {[
          { icon: "🤖", t: "AI Portfolio Builder", d: "Describe any thesis — \"nuclear energy & uranium\" or \"Simpsons-themed\" — and Grok-3 AI constructs 10 holdings with rationale, risk analysis, and macro outlook." },
      ...
      <div style={{ ...cardS(), marginBottom: 52, borderColor: C.accentBorder }}>
        ...
        <p style={{ color: C.dim, fontSize: 11, margin: "12px 0 0", lineHeight: 1.5 }}>Asset coverage: 10,000+ US-listed stocks & ETFs, 141 crypto assets (via Binance pairs), major commodities (gold, silver, oil via ETF proxies). Market hours: NYSE 9:30 AM–4:00 PM ET (stocks), 24/7 (crypto).</p>
      </div>
                ...
                <button onClick={async () => {
                  ...
                  try {
                    ...
                    if (UNSUPPORTED_TICKERS.has(sym)) {
                      setErr(`${sym} is not supported on our platform (Finnhub free tier limitation). For Bitcoin exposure, add BTC directly as a crypto holding.`);
                      setLoading(false);
                      return;
                    }
...
const UNSUPPORTED_TICKERS = new Set(["IBIT", "GBTC", "ETHE", "BITO", "ARKB", "FBTC", "BITB", "EZBC", "HODL", "BTCO", "DEFI", "BTCW", "RNDR", "RENDER"]);
const validateQuotePrice = (symbol, price, type) => {
  if (!price || price <= 0 || !isFinite(price)) return false;
  // Crypto sanity: flag if price seems suspiciously high for known rebrand/delist tokens
  if (KNOWN_PROBLEMATIC_TICKERS.has(symbol) && price > 5) {
    console.warn(`[Price Validator] ⚠ ${symbol} price $${price} flagged — known problematic ticker`);
    return false;
  }
  return true;
};
...
const crossValidateCryptoPrice = async (symbol, primaryPrice) => {
  ...
  const altSymbols = {
    "RNDR": "COINBASE:RNDR-USD", "RENDER": "COINBASE:RNDR-USD",
  };
  const alt = altSymbols[symbol];
  if (!alt) return primaryPrice; // No alt available, trust primary
  try {
    ...
    if (r.ok) {
      ...
      if (q && q.price > 0) {
        const diff = Math.abs(q.price - primaryPrice) / Math.max(q.price, primaryPrice);
        if (diff > 0.5) { // >50% divergence = stale data
          console.warn(`[Price Validator] ⚠ ${symbol}: primary $${primaryPrice} vs alt $${q.price} — ${(diff*100).toFixed(0)}% divergence, using alt`);
          return q.price;
        }
      }
    }
...
const fetchRealQuotes = async (holdings) => {
  ...
  const fhSymbols = needFetch.map(s => sym2fh[s]).join(",");
    ...
    if (r.ok) {
      ...
      if (data.keyMissing) {
        ...
        Object.entries(data.quotes).forEach(([fhSym, q]) => {
          const origSym = fh2sym[fhSym] || fhSym;
  ...
  const CRYPTO_FALLBACKS = {
    ...
    "BRETT": ["BINANCE:BRETTUSDT"], "POPCAT": ["BINANCE:POPCATUSDT"],
    "MOG": ["BINANCE:MOGUSDT"], "PONKE": ["BINANCE:PONKEUSDT"],
    ...
    const fetchLiveValues = async (isFirst = false) => {
      ...
      portfoliosRef.current.forEach((p, idx) => {
        ...
        const newHoldings = p.holdings.map(h => {
          const q = quotes[h.symbol];
          if (q && h.shares && h.shares > 0) {
            // RNDR fix: correct stale entry prices inline
            let effectiveEntry = h.entryPrice;
            let effectiveShares = h.shares;
            if ((h.symbol === "RNDR" || h.symbol === "RENDER") && h.entryPrice > 3 && q.price < 3) {
              const origAlloc = h.originalAllocation || h.allocation || (h.entryPrice * h.shares);
              effectiveEntry = q.price;
              effectiveShares = origAlloc / q.price;
            }
            // REAL BROKERAGE MATH: value = shares × current market price
            const costBasis = effectiveEntry ? effectiveEntry * effectiveShares : (h.originalAllocation || h.allocation);
            const isShort = h.action === "SHORT";
            let holdingVal;
            if (isShort) {
              holdingVal = costBasis + (effectiveEntry - q.price) * effectiveShares;
            } else {
              holdingVal = effectiveShares * q.price;
            }
            ...
            const baseline = boughtToday ? (effectiveEntry || q.prevClose) : q.prevClose;
            const dayChange = (baseline > 0) ? ((q.price - baseline) / baseline) * 100 : 0;
            liveTotal += holdingVal;
            return {
              ...h, noQuote: false,
              entryPrice: effectiveEntry, shares: effectiveShares,
        ...
        // Day-1 safety: if all holdings are flat (entry≈now), cap value at costBasis
        const allFlat = finalHoldings.every(h => !h.entryPrice || !h.livePrice || Math.abs(h.entryPrice - h.livePrice) / h.entryPrice < 0.001);
        const safeVal = (allFlat && newVal > costBasis * 1.005) ? costBasis : newVal;
        ...
        const newTracking = [...(p.trackingData || []), { ts: Date.now(), value: safeVal }].slice(-200);
        updatePortfolio(idx, { ...p, value: safeVal, costBasis, trackingData: newTracking, holdings: normalizedFinal, liveData: hasAnyQuote, transactions: allTxs, lastLiveUpdate: Date.now() });
...
function Leaderboard({ publicPortfolios, user, lbLoading, lbLastUpdate }) {
  ...
  return (
    <div className="page-container" style={{ maxWidth: 920, margin: "0 auto", padding: "36px 20px" }}>
      ...
      {!lbLoading && lbLastUpdate && <div style={{ textAlign: "right", marginBottom: 6 }}><span style={{ color: C.dim, fontSize: 9, fontFamily: mono }}>● Live · updated {Math.round((Date.now() - lbLastUpdate) / 1000)}s ago</span></div>}
                    ...
                    <div style={{ padding: 18 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10, marginBottom: 16 }}>
                        {(() => { const cashBal = p.cashBalance || p.cashPosition?.amount || 0; const divYield = (p.holdings || []).reduce((s, h) => { const dd = DIVIDEND_DATA[h.symbol]; return s + (dd ? dd.yield * (h.weight || 0) / 100 : 0); }, 0); return [
                          ...
                          { l: "P&L", v: `${p.gain >= 0 ? "+" : ""}${fmtUSD(Math.round(p.value - (p.costBasis || 1000000)))}`, c: p.gain >= 0 ? C.green : C.red },
                          ...
                          <div style={{ display: "grid", gridTemplateColumns: "38px 50px 1fr 70px 90px 70px 70px", padding: "10px 18px", alignItems: "center", fontSize: 12 }}>
                            ...
                            <div>
                              ...
                              <div style={{ display: "flex", gap: 8, marginTop: 2, flexWrap: "wrap" }}>
                                ...
                                {h.shares && <span style={{ color: C.dim, fontSize: 8.5, fontFamily: mono }}>{h.shares < 1 ? h.shares.toFixed(6) : fmt(h.shares, 2)} shares</span>}
                                {!h.entryPrice && !h.livePrice && <span style={{ color: C.gold, fontSize: 8, fontFamily: mono }}>⚠ No price data</span>}
                                {(() => { const dd = DIVIDEND_DATA[h.symbol]; return dd && dd.yield > 0 ? <span style={{ color: C.gold, fontSize: 8.5, fontFamily: mono }}>💰 {fmt(dd.yield * 100, 2)}% yield ({dd.freq === 4 ? "quarterly" : dd.freq === 12 ? "monthly" : dd.freq === 2 ? "semi-annual" : "annual"})</span> : null; })()}
                              </div>
                              {h.role && <span style={{ color: C.sub, fontSize: 9 }}>{h.role}{h.conviction ? ` · ${h.conviction} conviction` : ""}</span>}
                            </div>
                    ...
                    <div style={{ padding: 18 }}>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
                        ...
                        {p.factorExposure && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}` }}>
                          ...
                          {typeof p.factorExposure === "object" ? (() => {
...
function Roadmap({ go }) {
  const liveFeatures = [
    ...
    { icon: "↗", title: "Social Sharing", desc: "Share portfolios, positions, and thesis to X, Facebook, Instagram, and LinkedIn from 6+ share points" },
  ...
  return (
    <div className="page-container" style={{ maxWidth: 880, margin: "0 auto", padding: "36px 20px" }}>
      ...
      <div style={{ ...cardS(), textAlign: "center", padding: "32px 24px", marginTop: 20, borderColor: C.accent + "33", background: C.accentBg }}>
        ...
        <a href="mailto:support@etfsimulator.com" style={{ ...btnP(), textDecoration: "none", display: "inline-block", fontSize: 13 }}>support@etfsimulator.com →</a>
      </div>
    </div>
  );
}
...
function AppInner() {
  ...
  const getHashPage = () => { const h = window.location.hash.replace("#",""); if (h.includes("access_token") || h.includes("type=recovery") || h.includes("error_description")) return "home"; return validPages.includes(h) ? h : "home"; };
        ...
        const loadPortfolios = async (attempt = 1) => {
          try {
            ...
            if (data && data.length > 0) {
              ...
              // ══════ RNDR → LINK SWAP + ALLOCATION FIX ══════
              // RNDR/RENDER has pricing issues. Replace with LINK in all portfolios.
              // Also normalize allocations so holdings + cash = costBasis exactly.
              const needsSwap = cleaned.some(p => (p.holdings || []).some(h => h.symbol === "RNDR" || h.symbol === "RENDER"));
              const needsNorm = cleaned.some(p => {
                const cb = p.costBasis || 1000000;
                const total = (p.holdings || []).reduce((s, h) => s + (h.originalAllocation || h.allocation || 0), 0) + (p.cashBalance || p.initialCashBalance || 0);
                return total > 0 && Math.abs(total - cb) > 500;
              });
              if (needsSwap || needsNorm) {
                console.log("[Migration] Running RNDR→LINK swap + allocation normalization...");
                // Fetch LINK price for the swap
                let linkPrice = 0;
                try {
                  const r = await fetch("/api/quote?symbols=BINANCE:LINKUSDT");
                  if (r.ok) { const d = await r.json(); const q = d.quotes?.["BINANCE:LINKUSDT"]; if (q?.price > 0) linkPrice = q.price; }
                } catch (e) {}
                if (!linkPrice) { try { const r = await fetch("/api/quote?symbols=LINK"); if (r.ok) { const d = await r.json(); const q = d.quotes?.LINK || Object.values(d.quotes || {})[0]; if (q?.price > 0) linkPrice = q.price; } } catch(e){} }
                console.log("[Migration] LINK price:", linkPrice > 0 ? `$${linkPrice}` : "unavailable");

                const fixed = cleaned.map(p => {
                  let changed = false;
                  let holdings = (p.holdings || []).map(h => {
                    if (h.symbol === "RNDR" || h.symbol === "RENDER") {
                      changed = true;
                      const origAlloc = h.originalAllocation || h.allocation || 0;
                      const ep = linkPrice > 0 ? Math.round(linkPrice * 10000) / 10000 : null;
                      const sh = ep ? Math.round((origAlloc / ep) * 100000000) / 100000000 : null;
                      console.log(`[Migration] ${p.name}: ${h.symbol} → LINK, alloc $${origAlloc}, entry $${ep}, shares ${sh}`);
                      return { ...h, symbol: "LINK", name: "Chainlink", type: "crypto", description: "Decentralized oracle network connecting smart contracts to real-world data.", entryPrice: ep, shares: sh, livePrice: ep, liveValue: origAlloc, originalAllocation: origAlloc, thesisConnection: h.thesisConnection?.replace(/RNDR|Render/gi, "LINK/Chainlink") || "Chainlink provides critical oracle infrastructure for DeFi and cross-chain applications.", rationale: "Replaced RNDR (delisted ticker) with LINK — a blue-chip crypto with deep liquidity and strong fundamentals in the oracle/data infrastructure space." };
                    }
                    return h;
                  });

                  // Normalize allocations to match costBasis
                  const cb = p.costBasis || 1000000;
                  const cash = p.cashBalance || p.initialCashBalance || 0;
                  const holdingsSum = holdings.reduce((s, h) => s + (h.originalAllocation || h.allocation || 0), 0);
                  const total = holdingsSum + cash;
                  if (total > 0 && Math.abs(total - cb) > 500) {
                    changed = true;
                    const targetH = cb - cash;
                    const scale = targetH / holdingsSum;
                    console.log(`[Migration] ${p.name}: normalizing $${Math.round(total)} → $${cb} (scale ${scale.toFixed(4)})`);
                    holdings = holdings.map(h => {
                      const newAlloc = Math.round((h.originalAllocation || h.allocation || 0) * scale);
                      const newShares = h.entryPrice > 0 ? Math.round((newAlloc / h.entryPrice) * 100000000) / 100000000 : h.shares;
                      return { ...h, allocation: newAlloc, liveValue: newAlloc, originalAllocation: newAlloc, shares: newShares, weight: Math.round(newAlloc / cb * 1000) / 10 };
                    });
                    // Fix rounding
                    const adjSum = holdings.reduce((s, h) => s + h.allocation, 0);
                    const diff = targetH - adjSum;
                    if (diff !== 0 && holdings.length > 0) { const mi = holdings.reduce((bi, h, i) => h.allocation > holdings[bi].allocation ? i : bi, 0); holdings[mi].allocation += diff; holdings[mi].originalAllocation += diff; holdings[mi].liveValue += diff; }
                  }

                  if (changed) return { ...p, holdings, value: cb, _migrated: true };
                  return p;
                });
                ...
                setPortfolios(fixed);
                // Persist to Supabase
                fixed.filter(p => p._migrated).forEach(async (p) => {
                  if (!p.dbId) return;
                  try {
                    const pd = { ...(p.portfolio_data || p), holdings: p.holdings };
                    await supabase.from("portfolios").update({ portfolio_data: pd, holdings: p.holdings }).eq("id", p.dbId);
                    console.log("[Migration] ✓ Persisted:", p.name);
                  } catch (e) { console.warn("[Migration] Could not persist:", e.message); }
                });
              }
            } else {
    ...
    const pricePubs = async () => {
      ...
      const recalculated = pubs.map(p => {
        ...
        const newHoldings = holdings.map(h => {
          const q = quotes[h.symbol];
          if (!q || !q.price || q.price <= 0) { liveTotal += h.liveValue || h.allocation || 0; return h; }
          pricedCount++;
          const isShort = h.action === "SHORT";
          let holdingVal, shares, entryP;
          
          // RNDR fix: if entry price was stale (>$3 for known problematic ticker), correct it using original allocation
          const isRndrBad = (h.symbol === "RNDR" || h.symbol === "RENDER") && h.entryPrice > 3 && q.price < 3;
          
          if (h.shares > 0 && h.entryPrice > 0 && !isRndrBad) {
            shares = h.shares; entryP = h.entryPrice;
            holdingVal = isShort ? (entryP * shares) + (entryP - q.price) * shares : shares * q.price;
          } else {
            const alloc = h.originalAllocation || h.allocation || h.liveValue || 0;
            if (alloc <= 0) { liveTotal += 0; return { ...h, livePrice: q.price }; }
            // For RNDR with bad entry: use current price as entry so P&L starts fresh
            entryP = isRndrBad ? q.price : (h.entryPrice || q.prevClose || q.price);
            shares = alloc / entryP;
            holdingVal = isShort ? alloc + (entryP - q.price) * shares : shares * q.price;
          }
          const boughtToday = h.entryDate && new Date(h.entryDate).toDateString() === new Date().toDateString();
          const baseline = boughtToday ? (entryP || q.prevClose) : (q.prevClose || entryP);
          const dayChg = baseline > 0 ? ((q.price - baseline) / baseline) * 100 : 0;
          liveTotal += holdingVal;
          return { ...h, shares, entryPrice: entryP, originalAllocation: h.originalAllocation || h.allocation, livePrice: q.price, liveValue: Math.round(holdingVal), dailyChange: isFinite(dayChg) ? dayChg : 0 };
        });
        if (pricedCount === 0) return p;
        const cash = p.cashBalance || 0;
        let totalVal = Math.round(liveTotal + cash);
        const holdingsCost = newHoldings.reduce((s, hh) => {
          if (hh.entryPrice > 0 && hh.shares > 0) return s + hh.entryPrice * hh.shares;
          return s + (hh.originalAllocation || hh.allocation || 0);
        }, 0);
        const cb = p.costBasis > 0 ? p.costBasis : ((holdingsCost + (p.initialCashBalance || cash)) || 1000000);
        // Day-1 safety: if ALL holdings have 0% movement (entry≈now), force value = costBasis
        // This prevents fake gains from AI allocation rounding errors
        const allFlat = newHoldings.every(h => !h.entryPrice || !h.livePrice || Math.abs(h.entryPrice - h.livePrice) / h.entryPrice < 0.001);
        if (allFlat && totalVal > cb * 1.005) {
          console.log(`[LB-Engine] Day-1 cap: ${p.name} $${totalVal} → $${cb} (all holdings flat)`);
          totalVal = cb;
        }
        const gain = cb > 0 ? Math.round(((totalVal - cb) / cb) * 10000) / 100 : 0;
        return { ...p, holdings: newHoldings, value: totalVal, gain, costBasis: cb };
      });
        ...
        if (lbRuns.current <= 2 || lbRuns.current % 4 === 0) {
          ...
          const toUpdate = recalculated.filter(r => r.id && r.holdings?.length > 0).map(p => ({
            ...
            holdings: (p.holdings || []).map(h => ({
              ...
              livePrice: h.livePrice, liveValue: h.liveValue,
              originalAllocation: h.originalAllocation || h.allocation,
  ...
  const pages = {
    ...
    leaderboard: <Leaderboard publicPortfolios={allPublicPortfolios} user={user} lbLoading={lbLoading} lbLastUpdate={lbLastUpdate} />,
    learn: <Learn />,

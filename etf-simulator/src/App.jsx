...
const TICKER_SYMBOLS = [
  ...
  { symbol: "IBIT", label: "BTC/USD" }, { symbol: "ETHA", label: "ETH/USD" },
  { symbol: "GLD", label: "Gold" }, { symbol: "USO", label: "Crude Oil" },
  { symbol: "TLT", label: "10Y Treasury" }, { symbol: "VIXY", label: "VIX" },
  { symbol: "FXE", label: "EUR/USD" }, { symbol: "FXY", label: "USD/JPY" },
];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    supabase.from("portfolios").select("*").eq("is_public", true).order("created_at", { ascending: false }).limit(50)
      .then(({ data }) => { if (data) setPublicPortfolios(data.map(row => ({ ...row.portfolio_data, id: row.id, isPublic: true, creator: row.portfolio_data?.creator || "Anonymous" }))); });
  ...
  const savePortfolio = async (p, isPublic) => {
    ...
    const row = {
      ...
      time_horizon: p.userTimeHorizon || "",
      rebal_freq: p.userRebalFreq || p.rebalanceFrequency || "",
      is_public: isPublic,
      portfolio_data: { ...p, isPublic, creator: user.name },
    };
    const { data, error } = await supabase.from("portfolios").insert(row).select();
    if (error) { alert("Save failed: " + error.message); return; }
    if (data && data[0]) {
      setPortfolios((prev) => [...prev, { ...p, isPublic, id: data[0].id, dbId: data[0].id }]);
    }
  };

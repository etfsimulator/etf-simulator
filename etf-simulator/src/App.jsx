...
function Builder({ user, openAuth, savePortfolio, publishPortfolio }) {
  ...
  // Simulate price movement every 10 seconds (with 60s grace period on launch)
  useEffect(() => {
    if (!portfolio) return;
    const createdTs = portfolio.id || Date.now();
    const timer = setInterval(() => {
      // Grace period: no price drift for first 60 seconds after launch
      if (Date.now() - createdTs < 60000) return;
  ...
  const generate = async () => {
    ...
    try {
      ...
      setEditName(p.name || ""); setEditTicker(p.ticker || ""); setEditingWeights({});
              ...
              <div key={i} style={{ opacity: isExcluded ? 0.4 : 1, transition: "opacity .2s" }}>
                ...
                {openIdx === i && <div style={{ padding: "14px 18px 14px 18px", background: C.surface, borderBottom: `1px solid ${C.border}`, fontSize: 13, color: C.sub, lineHeight: 1.7 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 10, padding: "8px 12px", background: C.card, borderRadius: 6, border: `1px solid ${C.border}` }}>
                    ...
                    <input type="number" min="0" max="40" step="0.5" value={editingWeights[i] !== undefined ? editingWeights[i] : h.weight} onChange={e => { const w = parseFloat(e.target.value) || 0; setEditingWeights(prev => ({ ...prev, [i]: w })); const newAlloc = Math.round(w * 10000); const updated = { ...portfolio }; updated.holdings = [...updated.holdings]; updated.holdings[i] = { ...updated.holdings[i], weight: w, allocation: newAlloc }; setPortfolio(updated); }} style={{ width: 60, background: C.surface, border: `1px solid ${C.border}`, color: C.text, padding: "4px 6px", borderRadius: 4, fontSize: 12, fontFamily: mono, textAlign: "center", outline: "none" }} />
                    <span style={{ color: C.sub, fontSize: 11 }}>% = {fmtUSD(Math.round((editingWeights[i] !== undefined ? editingWeights[i] : h.weight) * 10000))}</span>
                  </div>
...
export default function App() {
  ...
  const savePortfolio = async (p, isPublic) => {
    ...
    if (!user?.id) { alert("Please sign in to save your portfolio."); return; }
    try {
      ...
      if (error) { console.error("Save error:", error); alert("Save failed: " + error.message); return; }
      if (data && data[0]) {

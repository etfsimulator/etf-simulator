...
const retryMissingQuotes = async (holdings, existingQuotes) => {
  ...
  const stillMissing = (holdings || []).filter(h => h.symbol && !quotes[h.symbol] && h.type !== "crypto");
    ...
    try {
      ...
      const r = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 200,
          messages: [
            { role: "system", content: "You are a stock price lookup tool. Return ONLY a JSON object with current stock prices. No explanation, no markdown fences, just the JSON." },
            { role: "user", content: `Return current stock prices as JSON for: ${symbolList}. Format: {"TICKER": price}. Example: {"X": 42.50}` }
          ]
        })
      });
      if (r.ok) {

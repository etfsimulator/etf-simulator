...
function Portfolios({ user, openAuth, portfolios, go, updatePortfolio, publicPortfolios }) {
  ...
  const saveEdits = async (idx, p) => {
    ...
    if (p.dbId) {
      try {
        const pd = { ...(p.portfolio_data || p), name: updated.name, ticker: updated.ticker, strategy: updated.strategy, fee: updated.fee, thesisEdited: updated.thesisEdited || p.thesisEdited || false };
        ...
        console.log("[Edit] ✓ Saved edits for", updated.name, strategyChanged ? "(thesis edited — locked)" : "");

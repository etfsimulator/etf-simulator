...
function AuthModal({ onClose, onAuth, initMode }) {
  ...
  const submit = async () => {
    ...
    if (mode === "resetPassword") {
      ...
      // Success — close modal and let auth state change handle login
      onClose();
      return;
    }
    if (mode === "forgot") {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === "PASSWORD_RECOVERY") {
        console.log("[Auth] Password recovery detected — opening reset form");
        setAuthMode("resetPassword");
        return; // Don't auto-login yet, let them set a new password first
      }
      if (session?.user) {

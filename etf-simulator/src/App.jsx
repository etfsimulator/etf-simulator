...
const fetchRealQuotes = async (holdings) => {
  ...
  await Promise.all(needFetch.slice(0, 25).map(async (sym) => {
    ...
    } catch (e) { console.warn(`[Quote] ✗ ${sym}: ${e.message || "fetch failed"}`); }
  }));
  if (needFetch.length > 0) {
    const success = Object.keys(quotes).length;
    const fromCache = unique.length - needFetch.length;
    if (success === 0 && needFetch.length > 0) console.warn("[Quotes] ⚠ All API calls failed — check VITE_FINNHUB_API_KEY or rate limits");
    else if (success > 0) console.log(`[Quotes] ✓ ${success} live, ${fromCache} cached, ${needFetch.length - (success - fromCache)} failed`);
  }
  return quotes;
};
  ...
  useEffect(() => {
    ...
    const fetchLiveValues = async (isFirst = false) => {
      ...
      if (quotedCount > 0) {
        ...
        console.warn("[Portfolios] ⚠ No quotes returned — API may be failing");
      }
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 4 }}>
            {liveLoading ? (
              ...
              <span style={{ color: quoteCount > 0 ? C.green : C.red, fontSize: 10, fontFamily: mono }}>
                {quoteCount > 0 ? "●" : "○"} {quoteCount} quotes · Updated {new Date(lastQuoteTs).toLocaleTimeString()} · Auto-refresh 30s
              </span>
            ) : (
              <span style={{ color: C.red, fontSize: 10, fontFamily: mono }}>○ No market data — check API key</span>
            )}
              ...
              <div key={idx} style={{ ...cardS(), padding: 0, overflow: "hidden" }}>
                ...
                <div className="port-card-header" onClick={() => { setExpandedIdx(isExpanded ? null : idx); setActiveTab("overview"); setCompareWith(null); }} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "18px 20px", cursor: "pointer", background: isExpanded ? C.surface : "transparent", transition: "background .15s" }}>
                  ...
                  <div className="port-card-right" style={{ textAlign: "right", minWidth: 140, flexShrink: 0 }}>
                    ...
                    <div style={{ color: C.dim, fontSize: 10.5, marginTop: 2 }}>Fee: {p.fee || 0}% · {(() => { const types = [...new Set((p.holdings || []).map(h => h.type))]; const openTypes = types.filter(t => isMarketOpen(t)); const allClosed = openTypes.length === 0; return allClosed ? <span style={{ color: "#6b7280", fontSize: 9 }}>● MARKETS CLOSED</span> : <span style={{ color: C.green, fontSize: 9 }}>● LIVE ({openTypes.join(", ")})</span>; })()}</div>

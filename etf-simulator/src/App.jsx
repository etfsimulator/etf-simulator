                  ...
                  <div style={{ borderTop: `1px solid ${C.border}` }}>
                    ...
                    {activeTab === "overview" && (
                      <div className="tab-content" style={{ padding: 20 }}>
                        ...
                        {p.thesis && <div style={{ marginTop: 14, padding: "10px 14px", background: C.surface, borderRadius: 8, border: `1px solid ${C.border}` }}>
                          <div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 4, letterSpacing: 0.3 }}>ORIGINAL PROMPT</div>
                          <p style={{ color: C.dim, fontSize: 11.5, margin: 0, lineHeight: 1.5, fontStyle: "italic" }}>"{p.thesis}"</p>
                        </div>}
                    ...
                    <div style={{ padding: 18 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10, marginBottom: 16 }}>
                        {(() => { const cashBal = p.cashBalance || p.cashPosition?.amount || 0; const divYield = (p.holdings || []).reduce((s, h) => { const dd = DIVIDEND_DATA[h.symbol]; return s + (dd ? dd.yield * (h.weight || 0) / 100 : 0); }, 0); return [
                          ...
                          { l: "P&L", v: `${fmtSign(p.gain, 2).startsWith("+") ? "+" : ""}${fmtUSD(Math.round(p.value * (p.gain / 100)))}`, c: p.gain >= 0 ? C.green : C.red },
...
function Contact({ go }) {
  ...
  const sendMessage = async () => {
    ...
    // 1. Save to Supabase contact_messages table (best-effort — table may not exist yet)
    try {
    ...
    } catch (e) { console.warn("[Contact] DB save failed (table may not exist):", e.message); }
    // 2. Open mailto: link to actually deliver the email
      ...
      <div style={{ ...cardS() }}>
        ...
        {sent ? (
          <div style={{ textAlign: "center", padding: "30px 20px" }}>
            ...
            <p style={{ color: C.sub, fontSize: 13.5 }}>Your email client should have opened with the message pre-filled. If it didn't, you can email us directly at <span style={{ color: C.accent }}>support@etfsimulator.com</span></p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <button onClick={sendMessage} disabled={!form.name || !form.email || !form.message} style={{ ...btnP(), fontSize: 13, padding: "12px", opacity: !form.name || !form.email || !form.message ? 0.4 : 1 }}>Send Message →</button>
          </div>
        )}
      </div>
    </div>
...
function AppInner() {
  ...
  // This is CRITICAL: without this, other users' portfolios stay frozen at $1M.
  // Fetches real quotes for ALL holdings across ALL public portfolios, then
  // recalculates live values using the SAME math as the user's own portfolio tracker.
  const pubRef = useRef([]);
  ...
  useEffect(() => {
    ...
    const pricePubs = async () => {
      if (dead) return;
      // Throttle: no more than once per 25s
      if (Date.now() - pubUpdateThrottle.current < 25000) return;
      const pubs = pubRef.current;
      if (!pubs.length) return;

      // Step 1: Collect every unique symbol from every public portfolio
      const holdingMap = {};
      ...
      // Step 2: Fetch real market quotes (same function as user's own portfolio tracker)
      let quotes;
      try {
        quotes = await fetchRealQuotes(symbols);
      } catch (e) {
        console.error("[LB-Engine] Quote fetch failed:", e.message);
        return;
      }
      if (dead) return;
      const qCount = Object.keys(quotes).length;
      if (qCount === 0) { console.warn("[LB-Engine] 0 quotes returned — skipping"); return; }

      // Step 3: Recalculate every portfolio with live prices
      const recalculated = pubs.map(p => {
        ...
        const newHoldings = holdings.map(h => {
          ...
          if (!q || !q.price || q.price <= 0) {
            // No valid quote: keep frozen
            liveTotal += h.liveValue || h.allocation || 0;
            return h;
          }
          ...
          if (h.shares > 0 && h.entryPrice > 0) {
            // ── STANDARD PATH: has real brokerage data ──
            shares = h.shares;
            entryP = h.entryPrice;
            if (isShort) {
              holdingVal = (entryP * shares) + (entryP - q.price) * shares;
            } else {
              holdingVal = shares * q.price;
            }
          } else {
            // ── LEGACY PATH: estimate shares from allocation ──
            ...
            shares = alloc / entryP;
            if (isShort) {
              holdingVal = alloc + (entryP - q.price) * shares;
            } else {
              holdingVal = shares * q.price;
            }
          }

          // Day change (same logic as portfolio tracker)
          const boughtToday = h.entryDate && new Date(h.entryDate).toDateString() === new Date().toDateString();
          ...
          return {
            ...
            originalAllocation: h.originalAllocation || h.allocation,
            livePrice: q.price,
            liveValue: Math.round(holdingVal),
            dailyChange: isFinite(dayChg) ? dayChg : 0
          };
        });
        ...
        const totalVal = Math.round(liveTotal + cash);

        // Cost basis: stored, or sum of (entryPrice × shares) + initial cash
        ...
        return { ...p, holdings: newHoldings, value: totalVal, gain: gain, costBasis: cb };
      });

      // Step 4: ALWAYS commit — even tiny changes matter for live feel
      if (!dead) {
        const changed = recalculated.filter((r, i) => r.value !== pubs[i]?.value).length;
        console.log("[LB-Engine] ✓ Done:", qCount, "quotes,", changed, "portfolios changed value");
        pubUpdateThrottle.current = Date.now();
        setPublicPortfolios(recalculated);
      }
    };

    // Run after 1.5s initial delay, then every 30s (matches user's own portfolio refresh rate)
    const t1 = setTimeout(() => { if (!dead) pricePubs(); }, 1500);
    const iv = setInterval(() => { if (!dead) pricePubs(); }, 30000);

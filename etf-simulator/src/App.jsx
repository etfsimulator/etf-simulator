            ...
            <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 998 }} onClick={() => !saving && setShowSaveModal(false)}>
              <div onClick={(e) => e.stopPropagation()} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "28px 24px", width: 420, maxWidth: "92vw" }}>
                ...
                <div style={{ fontSize: 11, color: C.dim, marginBottom: 14, padding: "6px 10px", background: C.surface, borderRadius: 6, border: `1px solid ${C.border}` }}>
                  {user ? <span><span style={{ color: C.green }}>●</span> Signed in as <span style={{ color: C.accent, fontFamily: mono }}>{user.username || user.email}</span> {user._noSession && <span style={{ color: C.red }}>(⚠ no session — sign out and back in)</span>}</span> : <span style={{ color: C.red }}>● Not signed in</span>}
                </div>
              ...
              <div key={idx} style={{ ...cardS(), padding: 0, overflow: "hidden" }}>
                ...
                <div className="port-card-header" onClick={() => { setExpandedIdx(isExpanded ? null : idx); setActiveTab("overview"); setCompareWith(null); }} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "18px 20px", cursor: "pointer", background: isExpanded ? C.surface : "transparent", transition: "background .15s" }}>
                  ...
                  <div className="port-card-right" style={{ textAlign: "right", minWidth: 140, flexShrink: 0 }}>
                    ...
                    <div style={{ color: C.dim, fontSize: 10.5, marginTop: 2 }}>Fee: {p.fee || 0}% · <span style={{ color: C.green, fontSize: 9 }}>● LIVE</span></div>
                    ...
                    {activeTab === "transactions" && (
                      <div className="mob-scroll" style={{ padding: "0", maxHeight: 400, overflowY: "auto" }}>
                        {txs.length === 0 ? (
                          ...
                          <>
                            <div style={{ display: "grid", gridTemplateColumns: "70px 80px 90px 1fr", padding: "8px 20px", borderBottom: `1px solid ${C.border}`, fontSize: 9, color: C.dim, fontFamily: mono, textTransform: "uppercase", letterSpacing: 0.4, minWidth: 420 }}>
                              <span>Action</span><span>Symbol</span><span>Amount</span><span>Rationale</span>
                            </div>
                            {[...txs].reverse().map((tx, i) => (
                              <div key={i} style={{ display: "grid", gridTemplateColumns: "70px 80px 90px 1fr", padding: "8px 20px", borderBottom: `1px solid ${C.border}`, alignItems: "start", fontSize: 12, minWidth: 420 }}>
                                <span style={{ ...badge(tx.type === "BUY" ? C.green : tx.type === "SELL" || tx.type === "TRIM" || tx.type === "LIQUIDATE" ? C.red : tx.type === "SHORT" ? "#f59e0b" : tx.type === "FEE" ? C.dim : C.teal), fontSize: 9 }}>{tx.type}</span>
                                <span style={{ color: C.text, fontFamily: mono, fontWeight: 600, fontSize: 11 }}>{tx.symbol}</span>
                                <span style={{ color: C.text, fontFamily: mono, fontSize: 11 }}>{fmtUSD(tx.amount || 0)}</span>
                                <div>
                                  <span style={{ color: C.sub, fontSize: 11, lineHeight: 1.4, display: "block" }}>{tx.reason || "—"}</span>
                                  {tx.ts && <span style={{ color: C.dim, fontSize: 9, fontFamily: mono }}>{new Date(tx.ts).toLocaleString()}</span>}
                                </div>
                              </div>
                            ))}
                          </>
      ...
      {sorted.length === 0 ? (
        ...
        <div className="mob-scroll" style={{ ...cardS(), padding: 0, overflow: "hidden" }}>
          ...
          {sorted.map((p, i) => {
            ...
            return (
            <div key={p.id}>
              <div onClick={() => setExpanded(isOpen ? null : p.id)} style={{ display: "grid", gridTemplateColumns: "46px 1fr 100px 100px 76px 70px 50px", padding: "12px 18px", borderBottom: `1px solid ${C.border}`, alignItems: "center", cursor: "pointer", background: isOpen ? C.surface : "transparent", transition: "background .12s", minWidth: 560 }}>
              ...
              {isOpen && (
                <div className="lb-expanded" style={{ padding: "14px 18px 14px 64px", background: C.surface, borderBottom: `1px solid ${C.border}`, minWidth: 560 }}>
                  {/* Thesis */}
                  {p.thesis && <div style={{ marginBottom: 10 }}><span style={{ color: C.accent, fontWeight: 700, fontSize: 10.5, fontFamily: mono }}>THESIS: </span><span style={{ color: C.sub, fontSize: 12, lineHeight: 1.4 }}>{(p.thesis || "").slice(0, 200)}{(p.thesis || "").length > 200 ? "…" : ""}</span></div>}
                  {/* Holdings */}
                  {p.holdings && p.holdings.length > 0 && (
                    <div style={{ marginBottom: 10 }}>
                      <span style={{ color: C.dim, fontWeight: 700, fontSize: 10, fontFamily: mono, marginBottom: 6, display: "block" }}>HOLDINGS ({p.holdings.length})</span>
                      <div className="holding-chips" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        {p.holdings.map(h => (
                          <div key={h.symbol} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 6, padding: "6px 10px", minWidth: 80 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                              <span style={{ ...badge(TC[h.type] || C.accent), fontSize: 7 }}>{h.type}</span>
                              <span style={{ color: C.text, fontFamily: mono, fontWeight: 700, fontSize: 11 }}>{h.symbol}</span>
                            </div>
                            <div style={{ color: C.dim, fontSize: 9, marginTop: 2 }}>{h.name}</div>
                            <div style={{ color: C.sub, fontFamily: mono, fontSize: 10, marginTop: 2 }}>{fmt(h.weight, 1)}% · {fmtUSD(h.allocation)}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
                    <span style={{ color: C.dim, fontSize: 11, fontFamily: mono }}>{p.holdingCount || p.holdings?.length || 10} holdings</span>
                    {p.riskProfile && <span style={{ color: C.dim, fontSize: 11, fontFamily: mono }}>Risk: {p.riskProfile}</span>}
                    {p.benchmark && <span style={{ color: C.dim, fontSize: 11, fontFamily: mono }}>Bench: {p.benchmark}</span>}
                    <span style={{ color: C.dim, fontSize: 11, fontFamily: mono }}>Published by <span style={{ color: C.accent }}>@{p.creator}</span></span>
                    <button onClick={(e) => { e.stopPropagation(); shareToX(`🏆 Check out "${p.name}" by @${p.creator} on the ETF Simulator leaderboard!\n\n${p.gain >= 0 ? "📈" : "📉"} ${p.gain >= 0 ? "+" : ""}${fmt(p.gain, 2)}% return | ${fmtUSD(p.value)} NAV\n${(p.holdings || []).slice(0, 4).map(h => "$" + h.symbol).join(" ")}\n\nBuild your own AI ETF free → etfsimulator.com`); }} style={{ background: "#000", color: "#fff", border: "none", borderRadius: 5, padding: "4px 12px", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 4, marginLeft: "auto" }}>
...
export default function App() {
  ...
  const doAuth = async (u) => {
    // CRITICAL: Verify Supabase actually has an active session before declaring the user logged in
    const { data: sessionCheck } = await supabase.auth.getSession();
    if (sessionCheck?.session) {
      console.log("[Auth] ✓ Session verified for user:", u.email, "uid:", sessionCheck.session.user.id);
      setUser({ ...u, id: sessionCheck.session.user.id }); // Use session's uid as authoritative
      setAuthMode(null);
    } else {
      console.error("[Auth] ✗ doAuth called but NO Supabase session exists. User appears logged in but can't save.");
      // Still set user so UI shows them, but warn
      setUser({ ...u, _noSession: true });
      setAuthMode(null);
    }
  };
  ...
  const savePortfolio = async (p, isPublic) => {
    console.log("[Save] === SAVE ATTEMPT START ===");
    console.log("[Save] User state:", user ? { id: user.id, email: user.email, _noSession: user._noSession } : "NULL");
    console.log("[Save] Portfolios count:", portfolios.length, "/ 3 max");
    console.log("[Save] Portfolio:", p?.name, p?.ticker, "Holdings:", p?.holdings?.length);
    ...
    try {
      // Step 1: Refresh session to ensure JWT is fresh
      console.log("[Save] Step 1: Refreshing session...");
      const { data: refreshData, error: refreshError } = await supabase.auth.refreshSession();
      if (refreshError) {
        console.error("[Save] Session refresh failed:", refreshError);
      }

      // Step 2: Verify session exists
      console.log("[Save] Step 2: Verifying session...");
      const { data: sessionData } = await supabase.auth.getSession();
      if (!sessionData?.session) {
        console.error("[Save] ✗ NO SESSION after refresh. The user is not truly authenticated.");
        console.error("[Save] This usually means email confirmation is ON in Supabase settings.");
        console.error("[Save] Fix: Supabase Dashboard → Authentication → Providers → Email → Disable 'Confirm email'");
        return "No active login session. Please sign out, sign back in, and try again. (If you just created an account, check your email for a confirmation link first.)";
      }

      const sessionUserId = sessionData.session.user.id;
      console.log("[Save] ✓ Session valid. Session user:", sessionUserId, "App user:", user.id);

      // Step 3: Use the SESSION's user ID (authoritative), not React state
      const authorizedUserId = sessionUserId;

      // Step 4: Clean and prepare data
      console.log("[Save] Step 3: Preparing data...");
      ...
      const portfolioData = {
        ...
        trackingData: (p.trackingData || []).slice(-100),
        userRiskProfile: p.userRiskProfile, userTimeHorizon: p.userTimeHorizon, userRebalFreq: p.userRebalFreq,
      };
      const row = {
        user_id: authorizedUserId, // Use session's UID, not React state
        name: p.name || "AI Portfolio", ticker: p.ticker || "",
      ...
      console.log("[Save] Step 4: Inserting into Supabase...", { user_id: authorizedUserId, name: row.name, is_public: isPublic });
      ...
      if (error) {
        ...
        if (error.message?.includes("row-level security") || error.code === "42501") {
          return "Permission denied by database security policy. Please sign out completely, sign back in, and try again.";
        }
        if (error.message?.includes("violates") || error.code === "23505") {
          return "A portfolio with this name may already exist. Try changing the name.";
        }
        return "Database error: " + (error.message || error.code || "Unknown error");
      }

      if (data && data.length > 0 && data[0]) {
        console.log("[Save] ✓ SUCCESS! Row ID:", data[0].id, "Name:", data[0].name);
      ...
      // Insert returned no error but also no data — this can happen with RLS
      console.warn("[Save] ⚠ Insert returned no error but empty data array. Possible RLS select issue.");
      console.warn("[Save] data:", data, "error:", error);
      return "Save may have succeeded but couldn't be confirmed. Try refreshing the page and checking My Portfolios.";

    } catch (err) {
      console.error("[Save] ✗ EXCEPTION:", err);
      return "Save failed: " + (err.message || "Network error. Check your internet connection and try again.");
    }
  };

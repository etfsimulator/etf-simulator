...
function AppInner() {
  ...
  // Updates ALL public portfolios with real-time market data every 60 seconds.
  // Without this, other users' portfolios show stale values from their last save.
  const publicPortfoliosRef = useRef([]);
  publicPortfoliosRef.current = publicPortfolios;
  useEffect(() => {
    if (publicPortfolios.length === 0) return;
    let cancelled = false;
    const updatePublicValues = async () => {
      const pubs = publicPortfoliosRef.current;
      if (!pubs.length || cancelled) return;
      // Collect unique symbols across ALL public portfolios
      const allSymbols = new Map(); // symbol → type
      pubs.forEach(p => (p.holdings || []).forEach(h => { if (h.symbol && !allSymbols.has(h.symbol)) allSymbols.set(h.symbol, h.type || "stock"); }));
      if (allSymbols.size === 0) return;
      // Fetch live quotes for all symbols in one batch
      const quotes = await fetchRealQuotes([...allSymbols.entries()].map(([s, t]) => ({ symbol: s, type: t })));
      if (cancelled || Object.keys(quotes).length === 0) return;
      // Recalculate each portfolio's live value
      ...
      const updated = pubs.map(p => {
        ...
        const updatedHoldings = holdings.map(h => {
          const q = quotes[h.symbol];
          if (q && h.shares && h.shares > 0) {
            // PRIMARY PATH: has shares + entry price (standard brokerage math)
            hasAnyQuote = true;
            const costBasis = h.entryPrice ? h.entryPrice * h.shares : (h.originalAllocation || h.allocation);
            const isShort = h.action === "SHORT";
            let holdingVal;
            ...
            const dayChange = (baseline > 0) ? ((q.price - baseline) / baseline) * 100 : 0;
            liveTotal += holdingVal;
            return { ...h, livePrice: q.price, liveValue: Math.round(holdingVal), dailyChange: isFinite(dayChange) ? dayChange : 0 };
          } else if (q && (h.allocation > 0 || h.liveValue > 0)) {
            // LEGACY PATH: no shares data — estimate from allocation / prevClose
            hasAnyQuote = true;
            const alloc = h.allocation || h.liveValue || 0;
            const estimatedShares = alloc / (h.entryPrice || q.prevClose || q.price);
            const holdingVal = estimatedShares * q.price;
            const boughtToday = h.entryDate && new Date(h.entryDate).toDateString() === new Date().toDateString();
            const baseline = boughtToday ? (h.entryPrice || q.prevClose) : q.prevClose;
            const dayChange = (baseline > 0) ? ((q.price - baseline) / baseline) * 100 : 0;
            liveTotal += holdingVal;
            return { ...h, livePrice: q.price, liveValue: Math.round(holdingVal), dailyChange: isFinite(dayChange) ? dayChange : 0, shares: estimatedShares, entryPrice: h.entryPrice || q.prevClose };
          } else {
            // No quote available — keep frozen value
            liveTotal += h.liveValue || h.allocation || 0;
            return h;
          }
        });
        if (!hasAnyQuote) return p;
        const cash = p.cashBalance || 0;
        const totalVal = liveTotal + cash;
        const cb = p.costBasis || holdings.reduce((s, hh) => s + ((hh.entryPrice && hh.shares) ? hh.entryPrice * hh.shares : (hh.originalAllocation || hh.allocation || 0)), 0) + (p.initialCashBalance || cash) || 1000000;
        const gain = cb > 0 ? Math.round(((totalVal - cb) / cb) * 10000) / 100 : 0;
        const newVal = Math.round(totalVal);
        if (newVal !== p.value) anyChanged = true;
        return { ...p, holdings: updatedHoldings, value: newVal, gain };
      });
      if (!cancelled && anyChanged) {
        console.log("[Leaderboard] ✓ Live prices updated for", updated.filter((u, i) => u.value !== pubs[i].value).length, "portfolios");
        setPublicPortfolios(updated);
      }
    };
    // First update after short delay (let UI render first), then every 60s
    const initial = setTimeout(() => { if (!cancelled) updatePublicValues(); }, 2000);
    const iv = setInterval(() => { if (!cancelled) updatePublicValues(); }, 60000);
    return () => { cancelled = true; clearTimeout(initial); clearInterval(iv); };
  }, [publicPortfolios.length]); // Only re-trigger when count changes (initial load or new publish)

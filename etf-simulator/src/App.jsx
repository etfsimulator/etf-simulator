...
const fetchRealQuotes = async (holdings) => {
  ...
  // ══════ GROK LIVE FALLBACK — for crypto Finnhub doesn't carry on any exchange ══════
  const stillMissingCrypto = needFetch.filter(sym => !quotes[sym] && CRYPTO_FINNHUB[sym]);
  if (stillMissingCrypto.length > 0) {
    // Check if we have a recent Grok price cached (5 min TTL for Grok prices)
    const GROK_CACHE_TTL = 300000; // 5 minutes
    const needGrok = stillMissingCrypto.filter(sym => {
      const cached = quoteCache.data[sym];
      if (cached && cached.source === "grok" && (Date.now() - (cached.fetchedAt || 0)) < GROK_CACHE_TTL) {
        quotes[sym] = cached; // Use cached Grok price
        return false;
      }
      return true;
    });
    if (needGrok.length > 0) {
      console.log("[Quotes] Grok live fallback for:", needGrok.join(", "));
      try {
        const symbolList = needGrok.slice(0, 5).join(", ");
        const r = await fetch("/api/generate", {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 200,
            messages: [
              { role: "system", content: "You are a cryptocurrency price lookup tool. Return ONLY a JSON object with current prices in USD. No explanation, no markdown fences, just the JSON." },
              { role: "user", content: `Return current crypto prices as JSON for: ${symbolList}. Format: {"TICKER": price}. Example: {"BRETT": 0.045, "MEW": 0.0023, "POPCAT": 0.15}` }
            ]
          })
        });
        if (r.ok) {
          ...
          for (const [sym, price] of Object.entries(prices)) {
            if (price > 0 && needGrok.includes(sym)) {
              quotes[sym] = { price, prevClose: price * 0.98, fetchedAt: now, source: "grok" };
              quoteCache.data[sym] = quotes[sym];
              quoteCache.ts[sym] = now;
              console.log(`[Quotes] ✓ Grok live: ${sym} → $${price}`);
            }
          }
        }
      } catch (e) { console.warn("[Quotes] Grok live fallback failed:", e.message); }
    }
  }
...
const retryMissingQuotes = async (holdings, existingQuotes) => {
  ...
  // ══════ GROK FALLBACK — Last resort for tickers Finnhub can't resolve ══════
  const stillMissing = (holdings || []).filter(h => h.symbol && !quotes[h.symbol]);
  if (stillMissing.length > 0) {
    console.log("[Quotes] Grok fallback for:", stillMissing.map(h => h.symbol).join(", "));
    try {
      const symbolList = stillMissing.slice(0, 5).map(h => h.symbol).join(", ");
      const isCrypto = stillMissing.some(h => h.type === "crypto");
      const r = await fetch("/api/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          max_tokens: 200,
          messages: [
            { role: "system", content: "You are a price lookup tool. Return ONLY a JSON object with current prices. No explanation, no markdown fences, just the JSON." },
            { role: "user", content: `Return current ${isCrypto ? "cryptocurrency" : "stock"} prices as JSON for: ${symbolList}. Format: {"TICKER": price}. Example: ${isCrypto ? '{"BRETT": 0.045, "MEW": 0.0023}' : '{"X": 42.50}'}. Use most recent trading price.` }
          ]
        })
      });
      if (r.ok) {
        ...
        const prices = JSON.parse(cleaned);
        for (const [sym, price] of Object.entries(prices)) {
          if (price > 0 && stillMissing.some(h => h.symbol === sym)) {
            quotes[sym] = { price, prevClose: price, fetchedAt: now, source: "grok" };
            quoteCache.data[sym] = quotes[sym];
            quoteCache.ts[sym] = now;
            console.log(`[Quotes] ✓ Grok fallback: ${sym} → $${price}`);
          }
        }
      }
    } catch (e) { console.warn("[Quotes] Grok fallback failed:", e.message); }
  }
  return quotes;
};

...
const isMarketOpen = (assetType) => {
  ...
  const now = new Date();
  // Convert to ET (UTC-5, rough — doesn't handle DST perfectly but good enough)
    ...
    const fetchAndUpdate = async () => {
      ...
      if (portfolio.createdAt) {
        ...
        portfolio.holdings.forEach((h, i) => {
          ...
          anyDivs = true;
          h.shares = (h.shares || 0) + divCheck.sharesAdded;
          h.lastDividendTs = Date.now();
          divTxBatch.push({
            ...
            reason: `Dividend: $${divCheck.dividendPerShare}/share × ${((h.shares || 0) - divCheck.sharesAdded).toFixed(4)} shares = $${divCheck.amount.toFixed(2)} → reinvested ${divCheck.sharesAdded.toFixed(6)} shares at $${cp.toFixed(2)} (${(divCheck.yieldAnnual * 100).toFixed(2)}% annual yield)`,
          });
        });
          ...
          {portfolio.holdings.length < 20 && (() => {
            ...
            <div style={{ ...cardS(), marginBottom: 14, borderColor: C.accentBorder }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, flexWrap: "wrap", gap: 6 }}>
                ...
                <span style={{ color: C.dim, fontSize: 10 }}>{activeCount}/20 positions · Type any ticker: stocks, ETFs, crypto, commodities</span>
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                <div style={{ flex: "0 0 90px" }}>
                  <div style={{ color: C.dim, fontSize: 9, fontFamily: mono, marginBottom: 3 }}>SYMBOL</div>
                  <input id="custom-sym" placeholder="e.g. BTC" style={{ background: C.surface, border: `1px solid ${C.border}`, color: C.text, padding: "7px 8px", borderRadius: 5, fontSize: 13, fontFamily: mono, fontWeight: 700, width: "100%", outline: "none", textTransform: "uppercase", boxSizing: "border-box" }} maxLength={8} onInput={e => { e.target.value = e.target.value.toUpperCase().replace(/[^A-Z0-9.-]/g, ""); }} />
                </div>
                <div style={{ flex: "1 1 140px", minWidth: 100 }}>
                  <div style={{ color: C.dim, fontSize: 9, fontFamily: mono, marginBottom: 3 }}>NAME (optional)</div>
                  <input id="custom-name" placeholder="Bitcoin" style={{ background: C.surface, border: `1px solid ${C.border}`, color: C.sub, padding: "7px 8px", borderRadius: 5, fontSize: 12, width: "100%", outline: "none", boxSizing: "border-box" }} />
                </div>
                <div style={{ flex: "0 0 90px" }}>
                ...
                <button onClick={async () => {
                  const symEl = document.getElementById("custom-sym");
    ...
    const fetchLiveValues = async () => {
      ...
      portfoliosRef.current.forEach((p, idx) => {
        ...
        const finalHoldings = newHoldings.map(h => {
          ...
          liveTotal += (divCheck.sharesAdded * currentPrice) - (h.liveValue || h.allocation); // Adjust for the value change
          liveTotal = newHoldings.reduce((s2, h2) => s2 + (h2.symbol === h.symbol ? newValue : (h2.liveValue || h2.allocation)), 0);
          return { ...h, shares: newShares, liveValue: Math.round(newValue), lastDividendTs: Date.now() };
        });
        // Recalculate liveTotal from finalHoldings
        ...
        const totalW = finalHoldings.reduce((s, h) => s + Math.max(0.1, h.currentWeight || h.weight), 0);
        const normalized = finalHoldings.map(h => ({ ...h, currentWeight: Math.round((Math.max(0.1, h.currentWeight || h.weight) / totalW) * 1000) / 10 }));
        const newTracking = [...(p.trackingData || []), { ts: Date.now(), value: newVal }].slice(-200);
        updatePortfolio(idx, { ...p, value: newVal, trackingData: newTracking, holdings: normalized, liveData: hasAnyQuote, transactions: allTxs });
      });
    };
    fetchLiveValues();
    const iv = setInterval(fetchLiveValues, 30000);
    return () => { cancelled = true; clearInterval(iv); };
              ...
              <div key={idx} style={{ ...cardS(), padding: 0, overflow: "hidden" }}>
                ...
                <div className="port-card-header" onClick={() => { setExpandedIdx(isExpanded ? null : idx); setActiveTab("overview"); setCompareWith(null); }} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "18px 20px", cursor: "pointer", background: isExpanded ? C.surface : "transparent", transition: "background .15s" }}>
                  ...
                  <div className="port-card-right" style={{ textAlign: "right", minWidth: 140, flexShrink: 0 }}>
                    ...
                    <div style={{ color: gain >= 0 ? C.green : C.red, fontSize: 14, fontFamily: mono, fontWeight: 700 }}>{gain >= 0 ? "+" : ""}{fmt(gain, 2)}%</div>
                      ...
                      <div className="tab-content" style={{ padding: 20 }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 10, marginBottom: 16 }}>
                          {(() => { const divYield = holdings.reduce((s, h) => { const dd = DIVIDEND_DATA[h.symbol]; return s + (dd ? dd.yield * (h.weight || 0) / 100 : 0); }, 0); return [
                            ...
                            { l: "P&L", v: `${gain >= 0 ? "+" : ""}${fmtUSD(Math.round(p.value - 1000000))}`, c: gain >= 0 ? C.green : C.red },
                    ...
                    <div style={{ padding: 18 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10, marginBottom: 16 }}>
                        {(() => { const cashBal = p.cashBalance || p.cashPosition?.amount || 0; const divYield = (p.holdings || []).reduce((s, h) => { const dd = DIVIDEND_DATA[h.symbol]; return s + (dd ? dd.yield * (h.weight || 0) / 100 : 0); }, 0); return [
                          ...
                          { l: "P&L", v: `${p.gain >= 0 ? "+" : ""}${fmtUSD(Math.round(p.value - 1000000))}`, c: p.gain >= 0 ? C.green : C.red },
  ...
  useEffect(() => {
    ...
    supabase.from("portfolios").select("*").eq("is_public", true).order("created_at", { ascending: false }).limit(50)
      .then(({ data, error }) => {
        ...
        if (data) setPublicPortfolios(data.map(row => { const pd = row.portfolio_data || {}; const cash = pd.cashBalance || pd.cashPosition?.amount || 0; const h = Array.isArray(pd.holdings) ? pd.holdings : (Array.isArray(row.holdings) ? row.holdings : []); const holdingsVal = h.reduce((s, hh) => s + (hh.liveValue || hh.allocation || 0), 0); const val = holdingsVal > 0 ? (holdingsVal + cash) : (pd.value || row.value || 1000000); return { ...pd, id: row.id, isPublic: true, creator: pd.creator || "Anonymous", value: val, gain: Math.round(((val - 1000000) / 1000000) * 10000) / 100, fee: pd.fee || row.fee || 0, holdingCount: h.length, holdings: h, thesis: pd.thesis || pd.strategy || row.thesis || "", transactions: Array.isArray(pd.transactions) ? pd.transactions : [], cashBalance: cash }; }));
      });

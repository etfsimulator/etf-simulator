...
// Call AFTER fetchRealQuotes to fill in gaps (e.g. ADRs, unusual tickers).
const retryMissingQuotes = async (holdings, existingQuotes) => {
  ...
  console.log("[Quotes] Retrying", missing.length, "missing symbols individually:", missing.map(h => h.symbol).join(", "));
  const quotes = { ...existingQuotes };
  const now = Date.now();
  for (const h of missing.slice(0, 10)) {
    // Try common ADR suffix patterns
    const trySymbols = [h.symbol];
    if (h.symbol === "ADS") trySymbols.push("ADDYY", "ADDSF"); // Adidas
    if (h.symbol === "UIPath") trySymbols.push("PATH"); // UiPath
    for (const sym of trySymbols) {
      try {
        const type = detectAssetType(sym);
        const fhSym = toFinnhub(sym, type);
        const r = await fetch(`/api/quote?symbols=${encodeURIComponent(fhSym)}`);
        if (r.ok) {
          const data = await r.json();
          if (data.quotes) {
            const q = data.quotes[fhSym] || Object.values(data.quotes)[0];
            if (q && q.price > 0) {
              quotes[h.symbol] = { ...q, fetchedAt: now };
              quoteCache.data[h.symbol] = quotes[h.symbol];
              quoteCache.ts[h.symbol] = now;
              console.log(`[Quotes] ✓ Retry got ${h.symbol} via ${sym}: $${q.price}`);
              break;
            }
          }
        }
      ...
      portfoliosRef.current.forEach((p, idx) => {
        ...
        const newHoldings = p.holdings.map(h => {
          ...
          if (q && h.shares && h.shares > 0) {
            ...
            return { ...h, liveValue: h.liveValue || h.allocation, noQuote: true };
          }
        });
                            ...
                            <div key={i} style={{ borderBottom: `1px solid ${C.border}`, minWidth: 560 }}>
                              ...
                              <div>
                                ...
                                <div style={{ display: "flex", gap: 8, marginTop: 2, flexWrap: "wrap" }}>
                                  ...
                                  {h.noQuote && <span style={{ color: C.gold, fontSize: 8, fontFamily: mono }}>⚠ No live quote</span>}
                                  {!h.entryPrice && !h.noQuote && <span style={{ color: C.dim, fontSize: 8, fontFamily: mono }}>Awaiting price data...</span>}
                ...
                <div style={{ borderTop: `1px solid ${C.border}`, background: C.surface }}>
                  ...
                  {lbTab === "thesis" && (
                    <div style={{ padding: 18 }}>
                      ...
                      {p.thesis && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}`, marginBottom: 14 }}>
                        <div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 6 }}>ORIGINAL PROMPT</div>
                        <p style={{ color: C.text, fontSize: 13, margin: 0, lineHeight: 1.6, fontStyle: "italic" }}>"{p.thesis}"</p>
                      </div>}
                      {/* AI-Generated Strategy */}
                      {p.strategy && p.strategy !== p.thesis && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}`, marginBottom: 14 }}>
                    ...
                    <div style={{ padding: 18 }}>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
                        ...
                        {p.incomeProjection && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}` }}>
                          <div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 6 }}>INCOME PROJECTION</div>
                        ...
                        {p.factorExposure && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}` }}>
                          ...
                          {typeof p.factorExposure === "object" ? <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                            {Object.entries(p.factorExposure).map(([k, v]) => <div key={k} style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: C.sub, fontSize: 11, textTransform: "capitalize" }}>{k.replace(/([A-Z])/g, " $1")}</span><span style={{ color: v === "high" ? C.green : v === "low" ? C.dim : C.gold, fontSize: 11, fontWeight: 600, fontFamily: mono }}>{v}</span></div>)}

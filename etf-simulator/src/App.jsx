  ...
  useEffect(() => {
        ...
        const loadPortfolios = async (attempt = 1) => {
          try {
            ...
            if (data && data.length > 0) {
              ...
              // ══════ RNDR/RENDER ENTRY PRICE FIX ══════
              // RNDR rebranded to RENDER in 2024. Old Finnhub pair returned stale $7+ prices.
              // Fix: fetch real RENDER price, recalculate shares to preserve original allocation.
              const needsRndrFix = cleaned.some(p => (p.holdings || []).some(h => 
                (h.symbol === "RNDR" || h.symbol === "RENDER") && h.entryPrice > 3
              ));
              if (needsRndrFix) {
                console.log("[RNDR Fix] Detected stale RNDR entry prices, correcting...");
                // Try multiple exchange symbols to get real RENDER price
                const trySymbols = ["BINANCE:RENDERUSDT", "BINANCE:RNDRUSDT", "COINBASE:RNDR-USD"];
                let realPrice = 0;
                for (const sym of trySymbols) {
                  try {
                    const r = await fetch(`/api/quote?symbols=${encodeURIComponent(sym)}`);
                    if (r.ok) {
                      const data = await r.json();
                      const q = data.quotes?.[sym] || Object.values(data.quotes || {})[0];
                      if (q && q.price > 0 && q.price < 50) { realPrice = q.price; console.log(`[RNDR Fix] Got real price from ${sym}: $${realPrice}`); break; }
                    }
                  } catch (e) { /* try next */ }
                }
                if (realPrice > 0) {
                  const fixed = cleaned.map(p => {
                    const fixedHoldings = (p.holdings || []).map(h => {
                      if ((h.symbol !== "RNDR" && h.symbol !== "RENDER") || h.entryPrice <= 3) return h;
                      const origAlloc = h.originalAllocation || h.allocation || (h.entryPrice * h.shares);
                      const newShares = origAlloc / realPrice;
                      console.log(`[RNDR Fix] ${p.name}: ${h.symbol} entry $${h.entryPrice} → $${realPrice}, shares ${h.shares} → ${newShares.toFixed(2)}, alloc $${origAlloc}`);
                      return { ...h, entryPrice: Math.round(realPrice * 10000) / 10000, shares: Math.round(newShares * 100000000) / 100000000, livePrice: realPrice, liveValue: Math.round(newShares * realPrice), originalAllocation: origAlloc, _rndrFixed: true };
                    });
                    if (fixedHoldings.some(h => h._rndrFixed)) {
                      return { ...p, holdings: fixedHoldings };
                    }
                    return p;
                  });
                  cleaned = fixed;
                  ...
                  fixed.filter(p => p.holdings.some(h => h._rndrFixed)).forEach(async (p) => {
                    if (!p.dbId) return;
                    try {
                      const pd = { ...(p.portfolio_data || p), holdings: p.holdings };
                      await supabase.from("portfolios").update({ portfolio_data: pd, holdings: p.holdings }).eq("id", p.dbId);
                      console.log("[RNDR Fix] ✓ Persisted fix for", p.name);
                    } catch (e) { console.warn("[RNDR Fix] Could not persist:", e.message); }
                  });
                } else {
                  console.warn("[RNDR Fix] Could not fetch real RENDER price, skipping fix");
                }
              }
            } else {
              setPortfolios([]);
    ...
    supabase.from("portfolios").select("*").eq("is_public", true).order("created_at", { ascending: false }).limit(50)
      .then(({ data, error }) => {
        ...
        // Inline RNDR fix: if any RNDR holding has stale entry price >$3, flag it for live re-pricing
        h = h.map(hh => { if ((hh.symbol === "RNDR" || hh.symbol === "RENDER") && hh.entryPrice > 3) { const origAlloc = hh.originalAllocation || hh.allocation || (hh.entryPrice * hh.shares); return { ...hh, _needsRndrFix: true, originalAllocation: origAlloc }; } return hh; });

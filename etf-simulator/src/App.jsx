  ...
  const submit = async () => {
    ...
    if (mode === "signup") {
      ...
      if (data.session) {
        ...
        // Try to auto-sign-in immediately (works if email confirm is disabled but session wasn't returned for some reason)
        ...
        if (signInData?.session) {
          ...
          // Email confirmation IS required — inform user
          return setErr("Account created! Please check your email to confirm your account, then sign in. (Check spam folder too.)");
        }
      }
    } else {
      ...
      setLoading(false);
      if (error) {
        if (error.message?.includes("Email not confirmed")) return setErr("Please confirm your email first. Check your inbox (and spam folder) for the confirmation link.");
        return setErr(error.message);
      }
      if (!data.session) return setErr("Sign-in succeeded but no session was created. Please try again.");
          ...
          {showSaveModal && (
            <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 998 }} onClick={() => !saving && setShowSaveModal(false)}>
              <div onClick={(e) => e.stopPropagation()} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "28px 24px", width: 420, maxWidth: "92vw" }}>
                ...
                {saveErr && <div style={{ background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.25)", borderRadius: 8, padding: "10px 14px", marginBottom: 14, color: C.red, fontSize: 12, lineHeight: 1.5 }}>⚠ {saveErr}</div>}
...
export default function App() {
  ...
  const savePortfolio = async (p, isPublic) => {
    ...
    try {
      // Verify Supabase session is active (critical for RLS)
      const { data: sessionData } = await supabase.auth.getSession();
      if (!sessionData?.session) {
        console.error("Save blocked: No active Supabase session. User may need to re-login.");
        return "Your session has expired. Please sign out and sign back in, then try saving again.";
      }
      console.log("[Save] User:", user.id, "Session:", sessionData.session.user.id, "Public:", isPublic);
      ...
      console.log("[Save] Inserting row for portfolio:", p.name, "Holdings:", holdingsClean.length);
      const { data, error } = await supabase.from("portfolios").insert(row).select();
      if (error) {
        console.error("[Save] Supabase error:", error);
        if (error.message?.includes("row-level security") || error.code === "42501") {
          return "Permission denied. Your session may have expired — please sign out and back in, then try again.";
        }
        return "Database error: " + (error.message || error.code || "Unknown");
      }
      if (data && data[0]) {
        console.log("[Save] Success! Row ID:", data[0].id);
      ...
      console.warn("[Save] Insert returned no data and no error — unusual");
      return "Save appeared to succeed but no confirmation was returned. Check My Portfolios.";
    } catch (err) {
      console.error("[Save] Exception:", err);
      return "Save failed: " + (err.message || "Unknown error. Check your internet connection.");
    }
  };

...
const PRICING_TIERS = [
  { name: "Free", price: "$0", period: "forever", active: true, features: ["AI-generated ETF portfolios (up to 20 holdings)", "5 AI generations/day (guest) · 25/day (signed in)", "$1M simulated starting capital", "Risk profile & time horizon controls", "Flexible rebalancing (daily to annually)", "Keep/remove individual AI picks + add custom holdings", "Auto-rebalancing engine", "Live simulated trading with real-time pricing", "Community leaderboard", "Save up to 3 portfolios", "Interactive educational courses"] },
    ...
    const fetchTicker = () => {
      Promise.all(TICKER_SYMBOLS.map(t =>
        finnhubFetch(`https://finnhub.io/api/v1/quote?symbol=${t.symbol}&token=${FINNHUB_KEY}`)
          .then(r => r.json())
          .then(q => {
            if (!q || !q.c || q.c <= 0 || !isFinite(q.c)) return null;
            const pc = (q.pc && q.pc > 0 && isFinite(q.pc)) ? q.pc : q.c; // Guard against 0 or NaN
            const pctChange = Math.round(((q.c - pc) / pc) * 10000) / 100;
            if (!isFinite(pctChange)) return null; // Safety net
        ...
        if (live.length > 0) {
          ...
          // No live data — use defaults with subtle simulated movement
          // Only simulate movement for asset types whose markets are open
          const sim = indicesRef.current.map(d => {
            // Determine if this ticker's market is likely open
            const isFx = d.symbol.includes("/") || d.symbol === "DXY Index";
            const isCommodity = ["Gold", "Silver", "Crude Oil"].includes(d.symbol);
            const assetType = isFx ? "commodity" : isCommodity ? "commodity" : "stock";
            if (!isMarketOpen(assetType)) return d; // Market closed — freeze
            return {
              ...d,
              value: +(d.value + (Math.random() - 0.5) * d.value * 0.001).toFixed(d.value > 100 ? 2 : 4),
              change: +((d.change || 0) + (Math.random() - 0.5) * 0.08).toFixed(2),
            };
          });
...
function AuthModal({ onClose, onAuth, initMode }) {
  ...
  const checkUsername = (val) => {
    ...
    checkUsernameRef.current = setTimeout(async () => {
      try {
        // Check both profiles names AND portfolio creators for uniqueness
        const [profilesRes, portfoliosRes] = await Promise.all([
          supabase.from("profiles").select("name").limit(500),
          supabase.from("portfolios").select("portfolio_data->creator").limit(500),
        ]);
        const taken = new Set();
        (profilesRes.data || []).forEach(r => { if (r.name) taken.add(r.name.toLowerCase()); });
        (portfoliosRes.data || []).forEach(r => { const c = r.creator || ""; if (c) taken.add(c.toLowerCase()); });
        setUsernameOk(!taken.has(clean.toLowerCase()));
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {user ? (
            <>{editingUn ? (
              ...
              <span className="hide-mobile" style={{ color: C.sub, fontSize: 12.5, cursor: "pointer" }} onClick={() => { setUnInput(user.username || user.name || ""); setEditingUn(true); }} title="Click to change username"><span style={{ color: C.green }}>{"●"}</span> {user.username || user.name} <span style={{ color: C.dim, fontSize: 9 }}>✎</span></span>
  ...
  return (
    <div className="page-container" style={{ maxWidth: 1060, margin: "0 auto", padding: "48px 20px" }}>
      <div style={{ textAlign: "center", marginBottom: 56 }}>
        ...
        <p className="hero-sub" style={{ color: C.sub, fontSize: 17, maxWidth: 600, margin: "0 auto 28px", lineHeight: 1.6 }}>
...
const getMarketStatus = (assetType) => {
  ...
  if (day === 0 || day === 6) return { open: false, label: "WEEKEND", color: "#6b7280" };
...
const DIVIDEND_DATA = {
  ...
  IBM: { yield: 0.031, freq: 4 }, INTC: { yield: 0.013, freq: 4 }, CSCO: { yield: 0.028, freq: 4 },
...
function Builder({ user, openAuth, savePortfolio, publishPortfolio, signOut }) {
  ...
  const handleSymbolSearch = useCallback((q) => {
    ...
    searchTimeout.current = setTimeout(async () => {
      try {
        const r = await finnhubFetch(`https://finnhub.io/api/v1/search?q=${encodeURIComponent(q)}&token=${FINNHUB_KEY}`);
  ...
  const generate = async () => {
    ...
    try {
      ...
      setPortfolio({ ...p, id: Date.now(), createdTs: Date.now(), thesis, value: 1000000, costBasis: 1000000, initialCashBalance: initCash, createdAt: new Date().toISOString(), trackingData: [{ ts: Date.now(), value: 1000000 }], rebalanceThreshold: 5, userRiskProfile: riskProfile, userTimeHorizon: timeHorizon, userRebalFreq: rebalFreq });
  ...
  const weeklyUpdate = async () => {
    ...
    try {
      ...
      setPortfolio({ ...p, id: portfolio.id || Date.now(), createdTs: portfolio.createdTs, thesis: portfolio.thesis || thesis, value: Math.round(newAllocTotal), costBasis: portfolio.costBasis || 1000000, initialCashBalance: portfolio.initialCashBalance || 0, createdAt: portfolio.createdAt || new Date().toISOString(), trackingData: [...(portfolio.trackingData || []), { ts: Date.now(), value: Math.round(newAllocTotal) }], rebalanceThreshold: 5, userRiskProfile: portfolio.userRiskProfile || riskProfile, userTimeHorizon: portfolio.userTimeHorizon || timeHorizon, userRebalFreq: portfolio.userRebalFreq || rebalFreq, lastUpdated: new Date().toISOString() });
      ...
      {portfolio && (
        <div>
            ...
            <div style={{ ...cardS(), marginBottom: 14, borderColor: C.accentBorder }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                <div style={{ flex: "1 1 280px", minWidth: 200, position: "relative" }} ref={searchRef}>
                  <div style={{ color: C.dim, fontSize: 9, fontFamily: mono, marginBottom: 3 }}>SEARCH TICKER OR NAME</div>
                  <input 
                    id="custom-sym" 
                    placeholder="Search... e.g. Solana, AAPL, Bitcoin" 
                    value={searchQuery}
                    autoComplete="off"
                    onChange={e => {
                  ...
                  {selectedSymbol && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
                      <span style={{ ...badge(detectTickerType(selectedSymbol) === "crypto" ? "#f59e0b" : detectTickerType(selectedSymbol) === "etf" ? "#3b82f6" : C.accent), fontSize: 8.5 }}>
                        {detectTickerType(selectedSymbol).toUpperCase()}
                      </span>
                ...
                <button onClick={async () => {
                  ...
                  if (!sym) { setErr("Search and select a ticker symbol first"); return; }
                  ...
                  try {
                    ...
                    const quotes = await fetchRealQuotes([{ symbol: sym, type }]);
                    const q = quotes[sym];
                    const entryPrice = q ? q.price : null;
          ...
          <div className="builder-actions" style={{ display: "flex", justifyContent: "flex-end", gap: 10, flexWrap: "wrap" }}>
            ...
            <button onClick={() => { setPortfolio(null); setThesis(""); setSaved(false); setExcludedIdx(new Set()); setCashBalance(0); setTransactions([]); setNavHistory([]); setLiveAllocations({}); setRebalDeadline(null); setRebalCountdown(""); setEditingWeights({}); setNameEdited(false); }} style={btnO()}>Start Over</button>
            <button onClick={() => shareToX(`🚀 Just built "${portfolio.name}" ${portfolio.ticker ? `($${portfolio.ticker})` : ""} with AI on ETF Simulator!\n\n${portfolio.holdings.length} holdings | ${portfolio.riskProfile || "moderate"} risk | ${portfolio.fee || 0}% fee\n💡 ${portfolio.holdings.slice(0, 4).map(h => "$" + h.symbol).join(" ")}\n\n${portfolio.strategy ? portfolio.strategy.slice(0, 80) + "..." : ""}\n\nBuild yours → etfsimulator.com`)} style={{ ...btnO(), display: "inline-flex", alignItems: "center", gap: 4 }}><span style={{ fontWeight: 800 }}>𝕏</span> Share to X</button>
            {saved ? <span style={{ ...btnP(), opacity: 0.5, cursor: "default" }}>✓ Saved</span> : <button onClick={() => { if (!user) { setPendingSave(true); openAuth("signin"); return; } setSaveErr(""); setShowSaveModal(true); }} style={btnP()}>Save Portfolio ◆</button>}
          </div>
...
if (FINNHUB_KEY === "demo") console.warn("[ETF Simulator] ⚠ Using Finnhub DEMO key — most quotes will fail. Set VITE_FINNHUB_API_KEY in your Vercel env vars. Get a free key at https://finnhub.io/register");
...
// Prevents duplicate Finnhub API calls across components
// ═══════════════════════════════════════════════════════════════════════
const quoteCache = { data: {}, ts: {} };
const QUOTE_CACHE_TTL = 15000; // 15s freshness

// Batch-fetch real quotes from Finnhub with caching. Returns { SYMBOL: { price, prevClose } }
const fetchRealQuotes = async (holdings) => {
  ...
  const needFetch = [];
  unique.forEach(sym => {
    if (quoteCache.data[sym] && (now - (quoteCache.ts[sym] || 0)) < QUOTE_CACHE_TTL) {
  ...
  await Promise.all(needFetch.slice(0, 25).map(async (sym) => {
    try {
...
function Portfolios({ user, openAuth, portfolios, go, updatePortfolio, publicPortfolios }) {
  ...
  const getPortfolioCostBasis = (p) => {
    const holdingsCost = (p.holdings || []).reduce((s, h) => {
      const cost = (h.entryPrice && h.shares) ? h.entryPrice * h.shares : (h.originalAllocation || h.allocation || 0);
      return s + cost;
    }, 0);
  ...
  useEffect(() => {
    ...
    const fetchLiveValues = async (isFirst = false) => {
      ...
      if (quotedCount > 0) {
        console.log("[Portfolios] Live quotes:", quotedCount, "symbols");
      ...
      portfoliosRef.current.forEach((p, idx) => {
        ...
        const newHoldings = p.holdings.map(h => {
          ...
          if (q && h.shares && h.shares > 0) {
            ...
            return { ...h, liveValue: h.liveValue || h.allocation };
          }
        });
        ...
        // Recompute weights after dividends (finalHoldings may have changed values)
        ...
        const now = Date.now();
        if (p.dbId && hasAnyQuote && (now - lastPersistTs.current > 180000)) {
    ...
    const iv = setInterval(() => fetchLiveValues(false), 30000);
    return () => { cancelled = true; clearInterval(iv); };
    ...
    <div className="page-container" style={{ maxWidth: 1000, margin: "0 auto", padding: "36px 20px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: C.sub, fontSize: 13, margin: 0 }}>{portfolios.length}/3 used (Free tier) · Values update in real time · Logged in as <span style={{ color: C.accent, fontFamily: mono }}>{user.username || user.name}</span></p>
          {liveLoading && <p style={{ color: C.gold, fontSize: 11, margin: "4px 0 0", fontFamily: mono }}>⏳ Fetching live market prices...</p>}
          {!liveLoading && lastQuoteTs && <p style={{ color: C.dim, fontSize: 10, margin: "4px 0 0", fontFamily: mono }}>● Last updated: {new Date(lastQuoteTs).toLocaleTimeString()}</p>}
        </div>
          ...
          {portfolios.map((p, idx) => {
            ...
            return (
              <div key={idx} style={{ ...cardS(), padding: 0, overflow: "hidden" }}>
                ...
                <div className="port-card-header" onClick={() => { setExpandedIdx(isExpanded ? null : idx); setActiveTab("overview"); setCompareWith(null); }} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "18px 20px", cursor: "pointer", background: isExpanded ? C.surface : "transparent", transition: "background .15s" }}>
                  ...
                  <div className="port-card-right" style={{ textAlign: "right", minWidth: 140, flexShrink: 0 }}>
                    ...
                    <div style={{ color: gain >= 0 ? C.green : C.red, fontSize: 14, fontFamily: mono, fontWeight: 700 }}>{gain >= 0 ? "+" : ""}{fmt(gain, 2)}% <span style={{ fontSize: 11, fontWeight: 600 }}>({gain >= 0 ? "+" : ""}{fmtUSD(Math.round(p.value - costBasis))})</span></div>
                    <div style={{ color: C.dim, fontSize: 10.5, marginTop: 2 }}>Fee: {p.fee || 0}% · {(() => { const types = [...new Set((p.holdings || []).map(h => h.type))]; const openTypes = types.filter(t => isMarketOpen(t)); const allClosed = openTypes.length === 0; return allClosed ? <span style={{ color: "#6b7280", fontSize: 9 }}>● MARKETS CLOSED</span> : <span style={{ color: C.green, fontSize: 9 }}>● LIVE ({openTypes.join(", ")})</span>; })()}{p.lastLiveUpdate && <span style={{ color: C.dim, fontSize: 8, marginLeft: 6 }}>Updated {Math.round((Date.now() - p.lastLiveUpdate) / 1000)}s ago</span>}</div>
                    {Math.abs(dayChange) > 0 && <div style={{ color: dayChange >= 0 ? C.green : C.red, fontSize: 10, fontFamily: mono, marginTop: 2 }}>Today: {dayChange >= 0 ? "+" : ""}{fmtUSD(Math.round(dayChange))}</div>}
                  ...
                  <div style={{ borderTop: `1px solid ${C.border}` }}>
                      ...
                      <div className="tab-content" style={{ padding: 20 }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 10, marginBottom: 16 }}>
                          {(() => { const divYield = holdings.reduce((s, h) => { const dd = DIVIDEND_DATA[h.symbol]; return s + (dd ? dd.yield * (h.weight || 0) / 100 : 0); }, 0); return [
                            ...
                            { l: "Total Return", v: `${gain >= 0 ? "+" : ""}${fmt(gain, 2)}%`, c: gain >= 0 ? C.green : C.red },
                            { l: "P&L", v: `${gain >= 0 ? "+" : ""}${fmtUSD(Math.round(p.value - costBasis))}`, c: gain >= 0 ? C.green : C.red },
                            { l: "Today", v: `${dayChange >= 0 ? "+" : ""}${fmtUSD(Math.round(dayChange))}`, c: dayChange >= 0 ? C.green : C.red },
                        ...
                        {/* Strategy & Thesis */}
                        {p.thesis && <div style={{ marginTop: 14, padding: "10px 14px", background: C.surface, borderRadius: 8, border: `1px solid ${C.border}` }}><div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 4 }}>INVESTMENT THESIS</div><p style={{ color: C.sub, fontSize: 12, margin: 0, lineHeight: 1.5 }}>{p.thesis}</p></div>}
                        {/* Share to X */}
                        <div className="share-row" style={{ marginTop: 14, padding: "12px 16px", background: `${C.accent}08`, borderRadius: 8, border: `1px solid ${C.accentBorder}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          <div>
                            <div style={{ color: C.text, fontSize: 12.5, fontWeight: 600, marginBottom: 2 }}>Share your portfolio performance</div>
                            <div style={{ color: C.dim, fontSize: 10.5 }}>Post your gains (or losses) to X and challenge friends to beat you</div>
                          </div>
                          <button onClick={() => shareToX(`📊 My AI-built ETF "${p.name}" ${p.ticker ? `($${p.ticker})` : ""}\n\n${gain >= 0 ? "📈" : "📉"} ${gain >= 0 ? "+" : ""}${fmt(gain, 2)}% return (${fmtUSD(p.value)} from ${fmtUSD(Math.round(costBasis))})\n🎯 ${p.holdings.length} holdings | ${p.fee || 0}% fee | ${p.riskProfile || "moderate"} risk\n💡 Top picks: ${p.holdings.slice(0, 3).map(h => "$" + h.symbol).join(" ")}\n\nBuild yours free → etfsimulator.com`)} style={{ background: "#000", color: "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap" }}>
                            <span style={{ fontSize: 14 }}>𝕏</span> Share to X
                          </button>
                        </div>
                      </div>
                    )}
                    ...
                    {activeTab === "holdings" && (
                      <div className="mob-scroll" style={{ padding: "0" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "38px 50px 1fr 70px 90px 80px 70px", padding: "8px 20px", borderBottom: `1px solid ${C.border}`, fontSize: 9, color: C.dim, fontFamily: mono, textTransform: "uppercase", letterSpacing: 0.4, minWidth: 540 }}>
                        ...
                        {p.holdings.map((h, i) => {
                          ...
                          const pnlPct = costBasis > 0 ? (pnl / costBasis) * 100 : 0;
                          return (
                            <div key={i} style={{ borderBottom: `1px solid ${C.border}`, minWidth: 540 }}>
                              <div style={{ display: "grid", gridTemplateColumns: "38px 50px 1fr 70px 90px 80px 70px", padding: "10px 20px", alignItems: "center", fontSize: 12 }}>
                              ...
                              <span style={{ color: (h.dailyChange || 0) >= 0 ? C.green : C.red, fontFamily: mono, fontSize: 11 }}>{(h.dailyChange || 0) >= 0 ? "+" : ""}{fmt(h.dailyChange || 0, 2)}%</span>
                              <span style={{ color: pnl >= 0 ? C.green : C.red, fontFamily: mono, fontSize: 11, fontWeight: 600 }}>{pnl >= 0 ? "+" : ""}{fmt(pnlPct, 1)}%</span>
                              </div>
                        ...
                        {cashBal > 0 && (
                          <div style={{ display: "grid", gridTemplateColumns: "38px 50px 1fr 70px 90px 80px 70px", padding: "10px 20px", borderBottom: `1px solid ${C.border}`, alignItems: "center", fontSize: 12, background: C.surface, minWidth: 540 }}>
                      ...
                      <div className="mob-scroll" style={{ padding: "0", maxHeight: 500, overflowY: "auto" }}>
                        {txs.length === 0 ? (
                          ...
                          <>
                            ...
                            {/* Column Headers */}
                            <div style={{ display: "grid", gridTemplateColumns: "70px 70px 90px 80px 80px 1fr", padding: "8px 20px", borderBottom: `1px solid ${C.border}`, fontSize: 9, color: C.dim, fontFamily: mono, textTransform: "uppercase", letterSpacing: 0.4, minWidth: 520 }}>
                              <span>Action</span><span>Symbol</span><span>Amount</span><span>Price</span><span>Shares</span><span>Details</span>
                            </div>
                            {[...txs].reverse().map((tx, i) => (
                              <div key={i} style={{ display: "grid", gridTemplateColumns: "70px 70px 90px 80px 80px 1fr", padding: "10px 20px", borderBottom: `1px solid ${C.border}`, alignItems: "start", fontSize: 12, minWidth: 520, background: i === 0 && tx.status === "FILLED" ? `${C.green}06` : "transparent" }}>
                    ...
                    {activeTab === "performance" && (
                      <div className="perf-tab" style={{ padding: 20 }}>
                        ...
                        <div style={{ display: "flex", gap: 6, marginBottom: 14, alignItems: "center", flexWrap: "wrap" }}>
                          <span style={{ color: C.dim, fontSize: 10, fontFamily: mono, letterSpacing: 0.4 }}>SIMULATION PERIOD:</span>
...
function Leaderboard({ publicPortfolios, user }) {
  ...
  const runLbBacktest = (p, months) => {
    ...
    for (let i = 0; i <= pts; i++) {
      const monthReturn = (p.holdings || []).reduce((s, h) => {
        ...
        return s + (h.weight / 100) * (typeRet + (Math.random() - 0.48) * typeVol);
      }, 0);
      nav = Math.round(nav * (1 + monthReturn));
      const spRet = 0.008 + (Math.random() - 0.48) * 0.04;
      sp = Math.round(sp * (1 + spRet));
      data.push({ month: i, value: nav });
      spData.push({ month: i, value: sp });
    }
    setLbBtData({ portfolio: data, sp: spData, name: p.name });
  };
    ...
    <div className="page-container" style={{ maxWidth: 920, margin: "0 auto", padding: "36px 20px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: C.sub, fontSize: 13, margin: 0 }}>Top community portfolios ranked by performance. Publish your ETF to compete.</p>
        </div>
          ...
          {sorted.map((p, i) => {
            ...
            <div key={p.id}>
              <div onClick={() => { if (isOpen) { setExpanded(null); } else { setExpanded(p.id); setLbTab("overview"); setLbBtData(null); } }} style={{ display: "grid", gridTemplateColumns: "46px 1fr 100px 100px 76px 70px 50px", padding: "12px 18px", borderBottom: `1px solid ${C.border}`, alignItems: "center", cursor: "pointer", background: isOpen ? C.surface : "transparent", transition: "background .12s", minWidth: 560 }}>
                ...
                <span style={{ color: C.accent, fontSize: 11.5, fontFamily: mono, fontWeight: 600 }}>@{p.creator}</span>
                <span style={{ color: C.text, fontFamily: mono, fontSize: 12 }}>{fmtUSD(p.value)}</span>
                <span style={{ color: p.gain >= 0 ? C.green : C.red, fontFamily: mono, fontSize: 12, fontWeight: 700 }}>{p.gain >= 0 ? "+" : ""}{fmt(p.gain, 2)}%</span>
              ...
              {isOpen && i < 10 && (
                <div style={{ borderTop: `1px solid ${C.border}`, background: C.surface }}>
                    ...
                    <div style={{ padding: 18 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10, marginBottom: 16 }}>
                        {(() => { const cashBal = p.cashBalance || p.cashPosition?.amount || 0; const divYield = (p.holdings || []).reduce((s, h) => { const dd = DIVIDEND_DATA[h.symbol]; return s + (dd ? dd.yield * (h.weight || 0) / 100 : 0); }, 0); return [
                          { l: "Portfolio Value", v: fmtUSD(p.value), c: C.text },
                          { l: "Total Return", v: `${p.gain >= 0 ? "+" : ""}${fmt(p.gain, 2)}%`, c: p.gain >= 0 ? C.green : C.red },
                          { l: "P&L", v: `${p.gain >= 0 ? "+" : ""}${fmtUSD(Math.round(p.value * (p.gain / 100)))}`, c: p.gain >= 0 ? C.green : C.red },
                          { l: "Cash Balance", v: fmtUSD(Math.round(cashBal)), c: cashBal > 0 ? C.teal : C.dim },
                          { l: "Expense Ratio", v: `${p.fee || 0}%/yr`, c: C.sub },
                          { l: "Dividend Yield", v: `${fmt(divYield * 100, 2)}%/yr`, c: divYield > 0 ? C.green : C.dim },
                          { l: "Holdings", v: `${(p.holdings || []).length} positions`, c: C.sub },
                          { l: "Risk Profile", v: p.riskProfile || "moderate", c: C.gold },
                          { l: "Manager", v: `@${p.creator}`, c: C.accent },
                      ...
                      <div style={{ marginTop: 14, display: "flex", justifyContent: "flex-end" }}>
                        <button onClick={(e) => { e.stopPropagation(); shareToX(`🏆 Check out "${p.name}" by @${p.creator} on the ETF Simulator leaderboard!\n\n${p.gain >= 0 ? "📈" : "📉"} ${p.gain >= 0 ? "+" : ""}${fmt(p.gain, 2)}% return | ${fmtUSD(p.value)} NAV\n${(p.holdings || []).slice(0, 4).map(h => "$" + h.symbol).join(" ")}\n\nBuild your own AI ETF free → etfsimulator.com`); }} style={{ background: "#000", color: "#fff", border: "none", borderRadius: 5, padding: "5px 14px", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 4 }}>
                          <span style={{ fontSize: 11 }}>𝕏</span> Share
                        </button>
                      </div>
                    </div>
                  )}
                      ...
                      {p.holdings.map((h, hi) => {
                        ...
                        <div key={hi} style={{ borderBottom: `1px solid ${C.border}`, minWidth: 520 }}>
                          <div style={{ display: "grid", gridTemplateColumns: "38px 50px 1fr 70px 90px 70px 70px", padding: "10px 18px", alignItems: "center", fontSize: 12 }}>
                            ...
                            <span style={{ color: C.accent, fontFamily: mono, fontWeight: 700, fontSize: 12 }}>{fmt(h.currentWeight || h.weight, 1)}%</span>
                  ...
                  {lbTab === "transactions" && (
                    <div className="mob-scroll" style={{ padding: 0, maxHeight: 400, overflowY: "auto" }}>
                      {(!p.transactions || p.transactions.length === 0) ? (
                        <div style={{ padding: "40px 18px", textAlign: "center", color: C.dim, fontSize: 12 }}>Transaction history is available when the portfolio manager saves their ETF with transaction logs.</div>
                      ) : (
                        <>
                  ...
                  {lbTab === "thesis" && (
                    <div style={{ padding: 18 }}>
                      {p.thesis ? (
                        <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}`, marginBottom: 14 }}>
                          <div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 6 }}>INVESTMENT THESIS</div>
                          <p style={{ color: C.text, fontSize: 13, margin: 0, lineHeight: 1.6 }}>{p.thesis}</p>
                        </div>
                      ) : <div style={{ color: C.dim, padding: 30, textAlign: "center" }}>No thesis provided for this portfolio</div>}
                      {p.fundSummary && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}`, marginBottom: 14 }}><div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 6 }}>FUND SUMMARY</div><p style={{ color: C.sub, fontSize: 12, margin: 0, lineHeight: 1.5 }}>{p.fundSummary}</p></div>}
                      {p.macroAnalysis && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}`, marginBottom: 14 }}><div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 6 }}>MACRO ANALYSIS</div><p style={{ color: C.sub, fontSize: 12, margin: 0, lineHeight: 1.5 }}>{p.macroAnalysis}</p></div>}
                      {p.rebalanceRules && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}` }}><div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 6 }}>REBALANCE RULES</div><p style={{ color: C.sub, fontSize: 12, margin: 0, lineHeight: 1.5 }}>{p.rebalanceRules}</p></div>}
                    </div>
                  )}
                  ...
                  {lbTab === "metrics" && (
                    <div style={{ padding: 18 }}>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
                        {p.riskAnalysis && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}` }}><div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 6 }}>RISK ANALYSIS</div><p style={{ color: C.sub, fontSize: 11, margin: 0, lineHeight: 1.5 }}>{p.riskAnalysis}</p></div>}
                        {p.incomeProjection && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}` }}><div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 6 }}>INCOME PROJECTION</div><p style={{ color: C.sub, fontSize: 11, margin: 0, lineHeight: 1.5 }}>{p.incomeProjection}</p></div>}
                        {p.esgConsiderations && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}` }}><div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 6 }}>ESG CONSIDERATIONS</div><p style={{ color: C.sub, fontSize: 11, margin: 0, lineHeight: 1.5 }}>{p.esgConsiderations}</p></div>}
                        {p.factorExposure && <div style={{ padding: "14px 18px", background: C.card, borderRadius: 8, border: `1px solid ${C.border}` }}><div style={{ color: C.dim, fontSize: 10, fontFamily: mono, marginBottom: 6 }}>FACTOR EXPOSURE</div><p style={{ color: C.sub, fontSize: 11, margin: 0, lineHeight: 1.5 }}>{p.factorExposure}</p></div>}
                      </div>
...
function Roadmap({ go }) {
  const liveFeatures = [
    { icon: "🧠", title: "AI Portfolio Generator", desc: "Grok-3 powered ETF construction with up to 20 holdings — AI picks + custom BUY/SHORT positions at real market prices" },
    ...
    { icon: "📚", title: "Investment Education", desc: "23 structured lessons from ETF basics to advanced portfolio theory and risk management" },
    { icon: "🪙", title: "141 Crypto Assets", desc: "Full crypto coverage: majors, DeFi, L1/L2, memecoins, AI tokens, gaming" },
    ...
    { icon: "💬", title: "Community Chat", desc: "24-hour ephemeral conversation on the Leaderboard — share strategies, debate picks, messages auto-delete daily" },
  ];
  const phases = [
    { num: 1, label: "Phase 1", timing: "Q2 2026", title: "Platform Hardening & Growth", status: "IN PROGRESS", desc: "Scale the user base, harden the infrastructure, and prepare for institutional features", color: C.accent, items: [
      ...
      { icon: "👥", title: "Community & Social", desc: "User profiles, portfolio cloning, strategy discussions with threaded replies, and peer-to-peer mentorship (basic 24hr chat is live)" },
...
function TermsOfService({ go }) {
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "36px 20px" }}>
      ...
      <p style={s}>Free tier users are limited to 5 AI portfolio generations per day without an account, or 25 per day with a free account, and 3 saved portfolios. We reserve the right to modify usage limits, features, and pricing at any time without notice.</p>
    ...
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "36px 20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 30 }}>
        <div style={{ ...cardS() }}>
          ...
          <p style={{ color: C.accent, fontSize: 14, margin: 0 }}>support@etfsimulator.com</p>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    supabase.from("portfolios").select("*").eq("is_public", true).order("created_at", { ascending: false }).limit(50)
      .then(({ data, error }) => {
        ...
        if (data) setPublicPortfolios(data.map(row => { const pd = row.portfolio_data || {}; const cash = pd.cashBalance || pd.cashPosition?.amount || 0; const h = Array.isArray(pd.holdings) ? pd.holdings : (Array.isArray(row.holdings) ? row.holdings : []); const holdingsVal = h.reduce((s, hh) => s + (hh.liveValue || hh.allocation || 0), 0); const val = holdingsVal > 0 ? (holdingsVal + cash) : (pd.value || row.value || 1000000); const cb = pd.costBasis || h.reduce((s, hh) => s + ((hh.entryPrice && hh.shares) ? hh.entryPrice * hh.shares : (hh.originalAllocation || hh.allocation || 0)), 0) + (pd.initialCashBalance || cash) || 1000000; return { ...pd, id: row.id, isPublic: true, creator: pd.creator || "Anonymous", value: val, costBasis: cb, gain: cb > 0 ? Math.round(((val - cb) / cb) * 10000) / 100 : 0, fee: pd.fee || row.fee || 0, holdingCount: h.length, holdings: h, thesis: pd.thesis || pd.strategy || row.thesis || "", transactions: Array.isArray(pd.transactions) ? pd.transactions : [], cashBalance: cash }; }));
      });
  ...
  const updateUsername = async (newName) => {
    ...
    try {
      await supabase.auth.updateUser({ data: { username: clean, name: clean } });
      // Also update profiles table if it exists
      try { await supabase.from("profiles").upsert({ id: user.id, name: clean }, { onConflict: "id" }); } catch (e) { /* profiles table may not exist */ }
      setUser(prev => ({ ...prev, username: clean, name: clean }));
      console.log("[Username] ✓ Updated to:", clean);
      return true;
  ...
  const savePortfolio = async (p, isPublic) => {
    ...
    if (portfolios.length >= 3) return "Free tier: max 3 saved portfolios. Delete one or upgrade to Pro for unlimited.";
  ...
  const pages = {
    ...
    builder: <Builder user={user} openAuth={openAuth} savePortfolio={savePortfolio} publishPortfolio={(p) => savePortfolio(p, true)} signOut={signOut} />,
    portfolios: <Portfolios user={user} openAuth={openAuth} portfolios={portfolios} go={setPage} updatePortfolio={updatePortfolio} publicPortfolios={allPublicPortfolios} />,
    leaderboard: <Leaderboard publicPortfolios={allPublicPortfolios} user={user} />,
    learn: <Learn />,
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}body{background:${C.bg}}
        ...
        @media(max-width:768px){
          ...
          .tab-bar button{white-space:nowrap!important;flex-shrink:0!important}
          ...
          .builder-actions button{width:100%!important}
        ...
        @media(max-width:480px){
          ...
          .tab-bar button{font-size:10px!important;padding:6px 8px!important}
      ...
      <footer style={{ borderTop: `1px solid ${C.border}`, padding: "32px 20px 22px", marginTop: 40 }}>
        <div className="footer-grid" style={{ maxWidth: 1060, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 24, marginBottom: 24 }}>
          ...
          <div>
            ...
            <span style={{ color: C.dim, fontSize: 12, display: "block", marginBottom: 5 }}>Support: support@etfsimulator.com</span>
          </div>
          <div>

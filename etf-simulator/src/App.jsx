        ...
        const loadPortfolios = async (attempt = 1) => {
          try {
            ...
            if (data && data.length > 0) {
              ...
              // IMPORTANT: Do NOT touch portfolio values, allocations, or any other holdings.
              // Only swap the symbol/name/entryPrice for RNDR holdings. Let the live pricing engine calculate real values.
              const needsSwap = cleaned.some(p => (p.holdings || []).some(h => h.symbol === "RNDR" || h.symbol === "RENDER"));
              if (needsSwap) {
                console.log("[Migration] Detected RNDR holdings, swapping to LINK...");
                let linkPrice = 0;
                try {
                ...
                if (linkPrice > 0) {
                  const fixed = cleaned.map(p => {
                    const hasRndr = (p.holdings || []).some(h => h.symbol === "RNDR" || h.symbol === "RENDER");
                    if (!hasRndr) return p;
                    const newHoldings = (p.holdings || []).map(h => {
                      if (h.symbol !== "RNDR" && h.symbol !== "RENDER") return h;
                      const origAlloc = h.originalAllocation || h.allocation || 0;
                      const ep = Math.round(linkPrice * 10000) / 10000;
                      const sh = Math.round((origAlloc / ep) * 100000000) / 100000000;
                      console.log(`[Migration] ${p.name}: ${h.symbol} → LINK, alloc $${origAlloc}, entry $${ep}, shares ${sh}`);
                      return { ...h, symbol: "LINK", name: "Chainlink", type: "crypto", description: "Decentralized oracle network connecting smart contracts to real-world data.", entryPrice: ep, shares: sh, livePrice: ep, originalAllocation: origAlloc, thesisConnection: h.thesisConnection?.replace(/RNDR|Render/gi, "LINK/Chainlink") || "Chainlink provides critical oracle infrastructure for DeFi and cross-chain applications.", rationale: "Replaced RNDR (delisted ticker) with LINK — a blue-chip crypto with deep liquidity and strong fundamentals." };
                    });
                    // Do NOT change p.value — let the live pricing engine recalculate from real prices
                    return { ...p, holdings: newHoldings, _migrated: true };
                  });
                  cleaned = fixed;
                  setPortfolios(fixed);
                  // Persist ONLY the holdings swap to Supabase — not values
                  fixed.filter(p => p._migrated).forEach(async (p) => {
                    if (!p.dbId) return;
                    try {
                      const pd = { ...(p.portfolio_data || p), holdings: p.holdings };
                      await supabase.from("portfolios").update({ portfolio_data: pd, holdings: p.holdings }).eq("id", p.dbId);
                      console.log("[Migration] ✓ Persisted LINK swap for", p.name);
                    } catch (e) { console.warn("[Migration] Could not persist:", e.message); }
                  });
                }
              }

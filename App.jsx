...
const fetchRealQuotes = async (holdings) => {
  ...
  unique.forEach(sym => {
    ...
    const effectiveTTL = (cachedEntry && cachedEntry.source && cachedEntry.source.startsWith("coingecko")) ? 60000 : QUOTE_CACHE_TTL;
  ...
  // ── COINGECKO PRIMARY — fetch these BEFORE Finnhub (Finnhub returns stale ATH prices for meme coins) ──
  const COINGECKO_PRIMARY_IDS = {
    "WIF": "dogwifcoin", "BONK": "bonk", "PEPE": "pepe", "FLOKI": "floki",
    "SHIB": "shiba-inu", "BOME": "book-of-meme", "PONKE": "ponke",
    "BRETT": "brett", "POPCAT": "popcat-sol", "MEW": "cat-in-a-dogs-world",
    "MOG": "mog-coin", "DOGE": "dogecoin",
  };
  const cgPrimaryNeeded = needFetch.filter(sym => COINGECKO_PRIMARY_IDS[sym] && !quotes[sym]);
  if (cgPrimaryNeeded.length > 0) {
    try {
      const ids = cgPrimaryNeeded.map(s => COINGECKO_PRIMARY_IDS[s]).join(",");
      const r = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${ids}&vs_currencies=usd&include_24hr_change=true`);
      if (r.ok) {
        const data = await r.json();
        for (const sym of cgPrimaryNeeded) {
          const cgId = COINGECKO_PRIMARY_IDS[sym];
          const d = data[cgId];
          if (d && d.usd > 0) {
            const price = d.usd;
            const change24h = d.usd_24h_change || 0;
            const prevClose = price / (1 + change24h / 100);
            const entry = { price, prevClose, fetchedAt: Date.now(), source: "coingecko-primary" };
            quotes[sym] = entry;
    ...
    } catch (e) { console.warn("[Quotes] CoinGecko primary fetch failed:", e.message); }
  }
  // Remove CG-primary symbols from needFetch so Finnhub doesn't overwrite with stale data
  ...
  // ══════ COINGECKO FALLBACK — for crypto Finnhub doesn't carry ══════
  // CoinGecko provides real, verified market data — never use LLMs for prices
  const COINGECKO_IDS = {
    "BRETT": "brett", "POPCAT": "popcat-sol", "MEW": "cat-in-a-dogs-world",
  ...
  const COINGECKO_CACHE_TTL = 60000; // 1 minute cache for CoinGecko
  const stillMissingCrypto = needFetch.filter(sym => !quotes[sym] && COINGECKO_IDS[sym]);
  if (stillMissingCrypto.length > 0) {
    // Check cache first
    const needCG = stillMissingCrypto.filter(sym => {
    ...
    if (needCG.length > 0) {
      console.log("[Quotes] CoinGecko fallback for:", needCG.join(", "));
      try {
        const ids = needCG.map(sym => COINGECKO_IDS[sym]).filter(Boolean).join(",");
        const r = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${ids}&vs_currencies=usd&include_24hr_change=true`);
        if (r.ok) {
          const data = await r.json();
          const now = Date.now();
          for (const sym of needCG) {
            ...
            if (cgData && cgData.usd > 0) {
              ...
              const prevClose = price / (1 + change24h / 100);
              quotes[sym] = { price, prevClose, fetchedAt: now, source: "coingecko" };
              quoteCache.data[sym] = quotes[sym];
              quoteCache.ts[sym] = now;
              console.log(`[Quotes] ✓ CoinGecko: ${sym} → $${price} (24h: ${change24h.toFixed(2)}%)`);
            }
          }
        }
      } catch (e) { console.warn("[Quotes] CoinGecko fallback failed:", e.message); }
    }
  }

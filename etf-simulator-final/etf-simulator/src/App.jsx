...
// NaN/null guard: renders "—" instead of "NaN" for bad values
const fmt = (v, d = 2) => (v == null || !isFinite(v)) ? "—" : new Intl.NumberFormat("en-US", { minimumFractionDigits: d, maximumFractionDigits: d }).format(v);
const fmtUSD = (v) => (v == null || !isFinite(v)) ? "$—" : new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(v);
// Signed format: handles -0 and near-zero values correctly (threshold = half a display unit)
const fmtSign = (v, d = 2) => { const n = Object.is(v, -0) ? 0 : (Math.abs(v) < 0.5 * Math.pow(10, -d) ? 0 : v); return (n >= 0 ? "+" : "") + fmt(n, d); };
      ...
      setLiveAllocations(prev => {
        ...
        portfolio.holdings.forEach((h, i) => {
          ...
          if (q && h.shares && h.shares > 0) {
            ...
            // Market closed and no quote — preserve last computed value (includes SHORT P&L)
            next[i] = prev[i] || h.liveValue || h.allocation;
          }
    ...
    try {
      ...
      p.holdings = p.holdings.map(h => {
        ...
        return {
          ...
          originalAllocation: h.allocation, // IMMUTABLE cost anchor — never overwritten after creation
        };
      });
...
const crossValidateCryptoPrice = async (symbol, primaryPrice) => {
  ...
  const CG_VALIDATE_IDS = {
    ...
    "WIF":"dogwifcoin",
    "BRETT":"based-brett",  // Base chain Brett — correct CoinGecko ID
    "POPCAT":"popcat",      // Solana Popcat — correct CoinGecko ID
    "MOG":"mog-coin",
...
const fetchRealQuotes = async (holdings) => {
  ...
  const BINANCE_PRIMARY_SYMBOLS = new Set([
    // Memecoins WITH confirmed Binance USDT pairs
    // EXCLUDED: BRETT, PONKE, MOG — no USDT pair on Binance
    // EXCLUDED: SPX — no SPXUSDT on Binance
    // EXCLUDED: SATS — listed as "1000SATS" on Binance (1000SATSUSDT), not "SATSUSDT" → 400 error
    "WIF","BONK","PEPE","FLOKI","SHIB","BOME","POPCAT","MEW","DOGE",
    "MEME","TURBO","NEIRO","GIGA","ORDI","BABYDOGE",
  ...
  if (binancePrimaryNeeded.length > 0) {
    try {
      ...
      if (r.ok) {
        ...
        if (r.status === 400) {
          // One invalid symbol poisons the whole batch — retry with rock-solid core tokens only
          console.warn("[Quotes] Binance batch 400 — retrying with core tokens...");
          const CORE_BINANCE = new Set(["BTC","ETH","SOL","BNB","DOGE","SHIB","PEPE","WIF","BONK","FLOKI","BOME","MEW","POPCAT","MEME","TURBO","FET","TAO","RENDER","UNI","LINK","AAVE"]);
          const coreNeeded = binancePrimaryNeeded.filter(s => CORE_BINANCE.has(s) && !quotes[s]);
          if (coreNeeded.length > 0) {
            try {
              const coreParam = JSON.stringify(coreNeeded.map(s => `${s}USDT`));
              const rCore = await fetch(`https://api.binance.com/api/v3/ticker/24hr?symbols=${encodeURIComponent(coreParam)}&type=MINI`);
              if (rCore.ok) {
                const coreTickers = await rCore.json();
                if (Array.isArray(coreTickers)) {
                  for (const t of coreTickers) {
                    const sym = t.symbol?.replace("USDT", "");
                    const price = parseFloat(t.lastPrice);
                    const prevClose = parseFloat(t.prevClosePrice) || price;
                    if (sym && price > 0) {
                      const entry = { price, prevClose, high: parseFloat(t.highPrice)||price, low: parseFloat(t.lowPrice)||price, fetchedAt: Date.now(), source: "binance-primary" };
                      quotes[sym] = entry; quoteCache.data[sym] = entry; lastGoodPrice[sym] = price; quoteCache.ts[sym] = Date.now();
                      console.log(`[Quotes] ✓ Binance retry: ${sym} → $${price}`);
                    }
                  }
                }
              }
            } catch (e2) { console.warn("[Quotes] Binance retry failed:", e2.message); }
          }
        } else {
  ...
  const COINGECKO_IDS = {
    ...
    "BRETT": "based-brett",   // Base chain Brett — NOT "brett" which is a different low-priced token
    "POPCAT": "popcat",       // Solana Popcat — "popcat-sol" does not exist on CoinGecko
      ...
      try {
        ...
        if (cgR.ok) {
          ...
          for (const sym of needCGFetch) {
            ...
            if (d && d.usd > 0) {
              const price = d.usd;
              // Guard: reject if CoinGecko price is >90% below seeded entry price.
              // A wrong CoinGecko ID returns a cheap unrelated token — do not poison lastGoodPrice.
              const seeded = lastGoodPrice[sym];
              if (seeded && seeded > 0 && price < seeded * 0.10) {
                console.warn(`[Quotes CoinGecko] ⚠ ${sym} $${price} is >90% below seeded entry $${seeded} — likely wrong CoinGecko ID or mismapped token. Skipping.`);
                continue;
              }
              ...
              quotes[sym] = entry;
              quoteCache.data[sym] = entry;
              lastGoodPrice[sym] = price; // persist last known-good only after validation
...
const retryMissingQuotes = async (holdings, existingQuotes) => {
  ...
  const COINGECKO_IDS_RETRY = {
    "BRETT": "based-brett", "POPCAT": "popcat", "MEW": "cat-in-a-dogs-world",
      ...
      if (r.ok) {
        ...
        for (const h of stillMissing) {
          ...
          if (cgData && cgData.usd > 0) {
            ...
            // Guard: reject if >90% below seeded entry price (catches wrong CoinGecko ID mapping)
            const seeded = lastGoodPrice[h.symbol];
            if (seeded && seeded > 0 && price < seeded * 0.10) {
              console.warn(`[Quotes CoinGecko retry] ⚠ ${h.symbol} $${price} >90% below seeded $${seeded} — skipping`);
              continue;
            }
      ...
      portfoliosRef.current.forEach((p, idx) => {
        ...
        const newHoldings = p.holdings.map(h => {
          ...
          if (q && h.shares && h.shares > 0) {
            ...
            // Has quote but no shares data (legacy portfolio) — estimate shares from originalAllocation/prevClose
            const costBasis = h.originalAllocation || h.allocation; // use immutable anchor, not current allocation
            const estimatedShares = costBasis / (q.prevClose || q.price);
            const isShort = h.action === "SHORT";
            const holdingVal = isShort
              ? costBasis + (h.entryPrice - q.price) * estimatedShares
              : estimatedShares * q.price;
            const pnl = holdingVal - costBasis;
            const pnlPct = costBasis > 0 ? (pnl / costBasis) * 100 : 0;
            const boughtToday = h.entryDate && new Date(h.entryDate).toDateString() === new Date().toDateString();
            const baseline = boughtToday ? (h.entryPrice || q.prevClose || 0) : (q.prevClose || 0);
                  ...
                  {(p.holdings || []).length > 0 && (
                    <div style={{ display: "flex", gap: 3, marginTop: 5, flexWrap: "wrap", alignItems: "center" }}>
                      {(p.holdings || []).slice(0, 6).map(h => (
                        <span key={h.symbol} style={{ fontSize: 8.5, fontFamily: mono, color: C.sub, background: C.card, border: `1px solid ${C.border}`, borderRadius: 3, padding: "1px 5px", letterSpacing: 0.2 }}>{h.symbol}</span>
                      ))}
                      {(p.holdings || []).length > 6 && <span style={{ fontSize: 8.5, fontFamily: mono, color: C.dim }}>+{(p.holdings || []).length - 6} more</span>}
                    </div>
                  )}
                </div>
                ...
                <div style={{ borderTop: `1px solid ${C.border}`, background: C.surface }}>
                        ...
                        <div key={hi} style={{ borderBottom: `1px solid ${C.border}`, minWidth: 520 }}>
                          <div style={{ display: "grid", gridTemplateColumns: "38px 50px 1fr 70px 90px 70px 70px", padding: "10px 18px", alignItems: "center", fontSize: 12 }}>
                            ...
                            <div>
                              ...
                              <div style={{ display: "flex", gap: 8, marginTop: 2, flexWrap: "wrap" }}>
                                {h.entryPrice && <span style={{ color: C.dim, fontSize: 8.5, fontFamily: mono }}>Entry: {fmtPrice(h.entryPrice)}</span>}
                                {h.livePrice && <span style={{ color: h.entryPrice ? (h.livePrice >= h.entryPrice ? C.green : C.red) : C.sub, fontSize: 8.5, fontFamily: mono }}>Now: {fmtPrice(h.livePrice)}</span>}
                                {h.shares && <span style={{ color: C.dim, fontSize: 8.5, fontFamily: mono }}>{h.shares < 1 ? h.shares.toFixed(6) : fmt(h.shares, 2)} shares</span>}
                            ...
                            <span style={{ color: h.dailyChange == null ? C.dim : (h.dailyChange >= 0 ? C.green : C.red), fontFamily: mono, fontSize: 11 }}>{h.dailyChange == null ? "—" : `${fmtSign(h.dailyChange, 2)}%`}</span>
                            <span style={{ color: !h.livePrice ? C.dim : (pnl >= 0 ? C.green : C.red), fontFamily: mono, fontSize: 11, fontWeight: 600 }}>
                              {!h.livePrice ? "—" : `${fmtSign(pnlPct, 1)}%`}
                            </span>
                          </div>
                  ...
                  {lbTab === "transactions" && (
                    <div className="mob-scroll" style={{ padding: 0, maxHeight: 400, overflowY: "auto" }}>
                      {(!p.transactions || p.transactions.length === 0) ? (
                        (() => {
                          const syntheticTxs = (p.holdings || [])
                            .map(h => ({
                              type: "BUY",
                              symbol: h.symbol,
                              amount: h.originalAllocation || h.allocation || 0,
                              pricePerShare: h.entryPrice || null,
                              reason: `Initial portfolio construction — ${h.weight || h.currentWeight || 0}% allocation`,
                              ts: p.createdAt || null,
                            }))
                            .filter(tx => tx.amount > 0);
                          if (syntheticTxs.length === 0) return (
                            <div style={{ padding: "40px 18px", textAlign: "center", color: C.dim, fontSize: 12 }}>
                              {p.isLiveFund ? "Transactions will appear here as dividends, fees, and stop-losses execute over time." : "No transaction history available for this portfolio."}
                            </div>
                          );
                          return (
                            <>
                              <div style={{ display: "flex", gap: 16, padding: "10px 18px", background: C.card, borderBottom: `1px solid ${C.border}`, flexWrap: "wrap" }}>
                                <span style={{ color: C.dim, fontSize: 10, fontFamily: mono }}>TOTAL ORDERS: <span style={{ color: C.text, fontWeight: 700 }}>{syntheticTxs.length}</span></span>
                                <span style={{ color: C.dim, fontSize: 10, fontFamily: mono }}>INVESTED: <span style={{ color: C.text, fontWeight: 700 }}>{fmtUSD(syntheticTxs.reduce((s, t) => s + (t.amount || 0), 0))}</span></span>
                                <span style={{ color: C.gold, fontSize: 10, fontFamily: mono, fontWeight: 600 }}>INITIAL CONSTRUCTION</span>
                              </div>
                              ...
                              {syntheticTxs.map((tx, ti) => (
                                <div key={ti} style={{ display: "grid", gridTemplateColumns: "60px 70px 80px 70px 1fr", padding: "8px 18px", borderBottom: `1px solid ${C.border}`, alignItems: "start", fontSize: 11, minWidth: 420 }}>
                                  <span style={{ ...badge(C.green), fontSize: 8 }}>BUY</span>
                                  ...
                                  <div>
                                    <span style={{ color: C.sub, fontSize: 10 }}>{tx.reason}</span>
                  ...
                  {lbTab === "metrics" && (
                    <div style={{ padding: 18 }}>
                      ...
                      {(() => {
                        const hasFm = (fm) => fm && typeof fm === "object" && (fm.marketCapValue || fm.peRatio || fm.revenueGrowth || fm.dividendYield);
      ...
      const recalculated = pubs.map(p => {
        ...
        const newHoldings = holdings.map(h => {
          ...
          if (h.shares > 0 && h.entryPrice > 0) {
            ...
            const shortBase = h.originalAllocation || h.allocation || (entryP > 0 && shares > 0 ? Math.round(entryP * shares) : 0);
            if (isShort && shortBase <= 0) console.warn(`[LB-Engine] SHORT ${h.symbol} has no valid cost anchor — shortBase=${shortBase}`);
        ...
        let feeTx = null;
        // Fee gate: p._lastFeeTs tracks last deduction time — checked on line below

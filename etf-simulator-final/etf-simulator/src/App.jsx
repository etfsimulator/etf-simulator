  ...
  return (
    <div className="page-container" style={{ maxWidth: 1060, margin: "0 auto", padding: "48px 20px" }}>
      ...
      <div className="grid3" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 52 }}>
        {[
          { icon: "🤖", t: "AI Portfolio Builder", d: "Describe any thesis — \"nuclear energy & uranium\" or \"Simpsons-themed\" — and Gemini 2.0 Flash constructs 10 holdings with rationale, risk analysis, and macro outlook." },
      ...
      <div className="grid4" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 52 }}>
        {[
          ...
          { n: "02", t: "AI Builds Your ETF", d: "Gemini 2.0 Flash constructs 10 holdings across stocks, ETFs, crypto, and commodities with detailed rationale." },
      ...
      <div style={{ ...cardS(), marginBottom: 52, borderColor: C.accentBorder }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div>
            <p style={{ color: C.sub, fontSize: 13, lineHeight: 1.7, margin: 0 }}><strong style={{ color: C.text }}>AI Engine:</strong> Portfolio generation powered by Google Gemini 2.0 Flash. Allocation based on institutional-grade analysis of risk, correlation, macro environment, and sector dynamics.</p>
          </div>
        </div>
...
function Builder({ user, openAuth, savePortfolio, publishPortfolio, signOut }) {
  ...
  const [dailyUses, setDailyUses] = useState(() => { try {
    // Use separate keys for guest vs signed-in users — prevents guest test uses burning pro quota
    const key = "etf_daily_uses_auth"; // will switch to auth key once user loads
    const stored = JSON.parse(localStorage.getItem("etf_daily_uses") || "{}");
  ...
  // ── Gemini 2.0 Flash — primary AI backend ──
  const GEMINI_KEY = "AIzaSyBtQjBU0NwBHIrKcpIZ1DIqgVPhqAZWGLs";
  const callGemini = async (systemPrompt, userPrompt, maxTokens = 8192, temperature = 0.7) => {
    ...
    if (!res.ok) {
      ...
      if (res.status === 400) throw new Error("GEMINI_BAD_REQUEST: " + errMsg);
  ...
  const generate = async () => {
    ...
    try {
      ...
      const raw = await callGemini(AI_SYSTEM_PROMPT, userPrompt, 8192, 0.7);
      ...
      if (e.message === "API_KEY_MISSING") setErr("Gemini API error — please try again (get a key at console.x.ai), or set VITE_API_PROXY_URL to use a server-side proxy.");
      else if (e.message?.includes("GEMINI_RATE_LIMIT")) setErr("Gemini rate limit hit — please wait a moment and try again.");
      else if (e.message?.includes("GEMINI_BAD_REQUEST")) setErr("Portfolio generation failed — try rephrasing your thesis.");
  ...
  const refreshHolding = async (idx) => {
    ...
    try {
      ...
      const rawH = await callGemini("You are an expert portfolio manager. Return ONLY valid JSON, no markdown fences.", refreshPrompt, 2048, 0.85);
  ...
  const weeklyUpdate = async () => {
    ...
    try {
      ...
      const rawUpdate = await callGemini(AI_SYSTEM_PROMPT, updatePrompt, 8192, 0.7);
...
const PRICE_CAPS = {
  ...
  "OP": 1.50,     // real ~$0.65
  "STX": 1.50,    // real ~$0.60
  // Memecoins — caps at ~10x current March 2026 price (blocks stale ATH ghosts)
  "DOGE": 1.6, "SHIB": 0.00012, "PEPE": 0.000085, "BONK": 0.00018,
  "FLOKI": 0.00085, "WIF": 12, "BRETT": 0.075, "POPCAT": 3.0,
  "MOG": 0.000018, "PONKE": 0.8, "BOME": 0.03, "MEW": 0.04,
  "MEME": 0.08, "TURBO": 0.02, "NEIRO": 0.004,
  // Delisted/rebranded
  "RNDR": 15, "RENDER": 15, "LUNA": 1, "UST": 0.05, "FTT": 5,
  // Small/mid-cap stocks in live funds — cap at ~5x realistic price (blocks ghost stale data)
  "NNE": 50,    // Nano Nuclear Energy — ~$8-15 range
  "OKLO": 80,   // Oklo Inc — ~$20-40 range
  "LEU": 200,   // Centrus Energy — ~$50-100 range
  "UEC": 20,    // Uranium Energy Corp — ~$4-8 range
  "BWXT": 250,  // BWX Technologies — ~$90-130 range
  "GEV": 600,   // GE Vernova — ~$250-400 range
  "MP": 40,     // MP Materials — ~$15-25 range
  "RKLB": 50,   // Rocket Lab — ~$15-30 range
  "CELH": 100,  // Celsius Holdings — ~$25-50 range
  "ON": 150,    // ON Semiconductor — ~$40-80 range
  "BNB": 5800,  // Binance Coin — ~$580 current, cap at 10x
};
...
const validateQuotePrice = (symbol, price, type) => {
  ...
  // 2. Generic flag for known problematic tickers without a cap
  if (KNOWN_PROBLEMATIC_TICKERS.has(symbol) && !cap && price > 5) {
    console.warn(`[Price Validator] ⚠ ${symbol} $${price} flagged — known problematic ticker`);
    return false;
  }
...
function Disclaimer({ go }) {
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "36px 20px" }}>
      ...
      <p style={s}>Market data is provided by third-party APIs (Finnhub, Google Gemini) and may be delayed, inaccurate, or incomplete. We do not guarantee the accuracy, timeliness, or completeness of any third-party data.</p>
    ...
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "36px 20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 20 }}>
        {[
          ...
          { icon: "🤖", title: "AI-Powered", desc: "Gemini 2.0 Flash builds portfolios with institutional-quality analysis" },
...
function AppInner() {
  ...
  const renderPage = () => {
    switch(page) {
      case "home":        return <Home go={setPage} openAuth={openAuth} user={user} />;
      ...
      case "leaderboard": return <Leaderboard publicPortfolios={allPublicPortfolios} user={user} lbLoading={lbLoading} lbLastUpdate={lbLastUpdate} />;
      ...
      default:            return <Home go={setPage} openAuth={openAuth} user={user} />;
    }
  };

            ...
            if (data && data.length > 0) {
              let cleaned = data.map(row => {
                ...
                const sanitizedHoldings = rawHoldings.map(h => {
                  ...
                  // Crypto: always strip DB livePrice — fetchAndUpdate will get real price live
                  if (h.type === "crypto") {
    ...
    supabase.from("portfolios").select("*").eq("is_public", true).order("created_at", { ascending: false }).limit(50)
      .then(({ data, error }) => {
        ...
        h = h.map(hh => {
          ...
          // Crypto: always strip DB livePrice — pricePubs will fetch real price within 500ms
          if (hh.type === "crypto") {
            return { ...hh, livePrice: null };
          }
          // Stocks: validate and keep, or fall back to entryPrice

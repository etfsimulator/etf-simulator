  ...
  useEffect(() => {
            ...
            if (data && data.length > 0) {
              let cleaned = data.map(row => {
                ...
                const sanitizedHoldings = rawHoldings.map(h => {
                  // Seed lastGoodPrice from entryPrice on load
                  ...
                  if (h.livePrice && !validateQuotePrice(h.symbol, h.livePrice, h.type)) {
                    ...
                    const fallback = lastGoodPrice[h.symbol];
                    return { ...h, livePrice: fallback || h.entryPrice || null, liveValue: h.originalAllocation || h.allocation };
                  }
                  return h;
                });
                return {
    ...
    supabase.from("portfolios").select("*").eq("is_public", true).order("created_at", { ascending: false }).limit(50)
      .then(({ data, error }) => {
        ...
        // Sanitize stale ghost prices persisted in DB + seed lastGoodPrice from entryPrice
        h = h.map(hh => {
          // Seed lastGoodPrice from entryPrice so relative check works on cold start
          if (hh.entryPrice && hh.entryPrice > 0 && !lastGoodPrice[hh.symbol]) {
            lastGoodPrice[hh.symbol] = hh.entryPrice;
          }
          if (hh.livePrice && !validateQuotePrice(hh.symbol, hh.livePrice, hh.type)) {
            ...
            // Use lastGoodPrice (now seeded from entryPrice) as fallback
            const fallback = lastGoodPrice[hh.symbol];
            return { ...hh, livePrice: fallback || hh.entryPrice || null, liveValue: hh.originalAllocation || hh.allocation };
          }
          return hh;
        });
        ...
        if (lbRuns.current <= 2 || lbRuns.current % 4 === 0) {
          ...
          const toUpdate = recalculated.filter(r => r.id && r.holdings?.length > 0).map(p => ({
            ...
            holdings: (p.holdings || []).map(h => {
              ...
              livePrice: safeLivePrice, liveValue: h.liveValue,
              originalAllocation: h.originalAllocation || h.allocation,

  ...
  useEffect(() => {
    ...
    supabase.from("portfolios").select("*").eq("is_public", true).order("created_at", { ascending: false }).limit(50)
      .then(({ data, error }) => {
        ...
        // ALWAYS recompute costBasis from entryPrice*shares — never trust stored value which may reflect stale prices
        const cb = h.reduce((s, hh) => s + ((hh.entryPrice > 0 && hh.shares > 0) ? hh.entryPrice * hh.shares : (hh.originalAllocation || hh.allocation || 0)), 0) + (pd.initialCashBalance || (pd.cashBalance !== undefined ? 0 : cash)) || 1000000;
        // Use originalAllocation for initial value (before live pricing engine runs) — avoids showing gains based on stale persisted liveValues
        const holdingsVal = h.reduce((s, hh) => s + (hh.originalAllocation || hh.allocation || 0), 0);
  ...
  useEffect(() => {
    ...
    const pricePubs = async () => {
      ...
      const recalculated = pubs.map(p => {
        ...
        const cb = (holdingsCost + (p.initialCashBalance || 0)) || 1000000;

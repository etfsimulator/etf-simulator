    ...
    const pricePubs = async () => {
      ...
      const recalculated = pubs.map(p => {
        ...
        finalHoldings = finalHoldings.map(h => {
          ...
          const freqMs = divData.frequency === "monthly" ? 30*24*3600*1000/30 : divData.frequency === "quarterly" ? 90*24*3600*1000/30 : 365*24*3600*1000/30;
          const simFreqMs = freqMs * (SIM_MONTH_MS / (30*24*3600*1000));
          if (lastDiv === 0 || (Date.now() - lastDiv) < simFreqMs) return h;
          const annualDivPerShare = h.livePrice * divData.yield;
          const periodsPerYear = divData.frequency === "monthly" ? 12 : divData.frequency === "quarterly" ? 4 : 1;

...
const TICKER_SYMBOLS = [
  ...
  // GLD~$278→Gold~2933, SLV~$30→Silver~32, USO~$70→WTI~67 (updated March 2026)
...
function Leaderboard({ publicPortfolios, user, lbLoading, lbLastUpdate }) {
  ...
  const runLbBacktest = (p, months) => {
    ...
    // Seeded RNG — derive stable seed from portfolio identity so chart is
    // identical on every open (no random chart-flicker during demos).
    const seed = (p.id ? String(p.id).split("").reduce((a, c) => a + c.charCodeAt(0), 0) : 0)
               + (p.name ? p.name.split("").reduce((a, c) => a + c.charCodeAt(0), 0) : 0)
               + 42;
    for (let i = 0; i <= pts; i++) {
      const regime = (sr(i * 3 + seed * 2 + 7) - 0.5) * 0.8;

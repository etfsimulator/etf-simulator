      ...
      <div style={{ ...cardS(), marginBottom: 20 }}>
        ...
        {!thesis.trim() && (
          <div style={{ marginBottom: 14 }}>
            <div style={{ color: C.dim, fontSize: 10, fontFamily: mono, letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>✦ Need inspiration? Click any to get started</div>
...
const PRICE_CAPS = {
  // AI tokens — Finnhub returns stale ATH prices
  "FET": 0.80,    // real ~$0.14, Finnhub ghost = $1.23
  "AGIX": 0.50,   // real ~$0.08
  "OCEAN": 0.80,  // real ~$0.25
  "TAO": 800,     // real ~$280
  "INJ": 30,      // real ~$10
  "SEI": 0.50,    // real ~$0.18
  "TIA": 5,       // real ~$2
...
const validateQuotePrice = (symbol, price, type) => {
  ...
  // Per-token price cap — rejects stale ATH ghost prices from Finnhub or stale DB values
  const cap = PRICE_CAPS[symbol];
  if (cap && price > cap) {
    console.warn(`[Price Validator] ⚠ ${symbol} price $${price} exceeds cap $${cap} — rejecting stale ghost price`);
    return false;
  }
  // Generic fallback: flag suspiciously high prices for other known problematic tickers
  if (KNOWN_PROBLEMATIC_TICKERS.has(symbol) && !cap && price > 5) {
  ...
  useEffect(() => {
    ...
    supabase.from("portfolios").select("*").eq("is_public", true).order("created_at", { ascending: false }).limit(50)
      .then(({ data, error }) => {
        ...
        // Sanitize stale ghost prices persisted in DB (e.g. FET=$1.23 from Finnhub)
        h = h.map(hh => {
    ...
    const pricePubs = async () => {
      ...
      const recalculated = pubs.map(p => {
        ...
        const newHoldings = holdings.map(h => {
          const q = quotes[h.symbol];
          // No quote — preserve last known liveValue (don't reset to allocation which breaks historical gain)
        ...
        // Cost basis = SEED_CAPITAL ($1M) always — every portfolio starts with $1,000,000
        // This is the only correct baseline: "you started with $1M, you now have $X"
        const SEED = 1000000;
        const cb = SEED;
        const gain = Math.round(((totalVal - cb) / cb) * 10000) / 100;
